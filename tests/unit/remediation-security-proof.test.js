...
describe('remediation security proof', () => {
  it('refuses path and risk claims when reanalysis is incomplete', () => {
    const proof = deriveRemediationSecurityProof({
      remediationId: 'r1', executionId: 'e1', findingId: 'finding-1',
      evidenceId: 'ev1', evidenceHash: 'hash', verification: verified,
      beforePaths: before, afterPaths: after, afterComplete: false,
      riskBefore: { score: 100 }, riskAfter: { score: 25 }, reanalysisEventId: null
    });
  ...
  it('only claims path removal after complete fresh analysis with lineage', () => {
    const proof = deriveRemediationSecurityProof({
      remediationId: 'r1', executionId: 'e1', findingId: 'finding-1',
      evidenceId: 'ev1', evidenceHash: 'hash', verification: verified,
      beforePaths: before, afterPaths: after, afterComplete: true,
      riskBefore: { score: 100 }, riskAfter: { score: 25 }, reanalysisEventId: 'event-1'
    });
    expect(proof.pathDiff.removedCount).toBe(1);
    expect(proof.pathImpactClaimed).toBe(true);
    expect(proof.risk.delta).toBe(-75);
  ...
  it('does not claim security reduction when provider verification fails', () => {
    const proof = deriveRemediationSecurityProof({
      remediationId: 'r1', executionId: 'e1', findingId: 'finding-1',
      evidenceId: 'ev1', evidenceHash: 'hash',
      verification: { outcome: 'VERIFICATION_FAILED' },
