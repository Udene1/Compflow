...
describe('remediation security impact claim boundary', () => {
  ...
  it('never treats the current graph as a completed after-state', () => {
    const proof = deriveRemediationSecurityProof({ remediationId: 'rem-1', executionId: 'exec-1', findingId: 'f-1', controlEvidenceId: 'ev-control', controlEvidenceHash: 'control-hash', verification, beforePaths: before, afterPaths: [], afterComplete: false });
  ...
  it('only emits a security reduction claim with complete fresh reanalysis lineage', () => {
    const proof = deriveRemediationSecurityProof({ remediationId: 'rem-1', executionId: 'exec-1', findingId: 'f-1', controlEvidenceId: 'ev-control', controlEvidenceHash: 'control-hash', reanalysisEvidenceId: 'ev-reanalysis', reanalysisEvidenceHash: 'reanalysis-hash', verification, beforePaths: before, afterPaths: [], afterComplete: true, riskBefore: { score: 100 }, riskAfter: { score: 50 }, reanalysisEventId: 'evt-1' });
