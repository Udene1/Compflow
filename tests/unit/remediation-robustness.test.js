...
describe('Phase 2 — Remediation Robustness Engine', () => {
    ...
    describe('AWS Remediation Blast Radius & Dry-Run Safety', () => {
        const mockCredentials = { accessKeyId: 'AKIA_MOCK_TEST_KEY', secretAccessKey: 'MOCK_SECRET_ACCESS_KEY', region: 'us-east-1' };
        it('should execute dry-run for S3 Public Access with structured response', async () => {
            ...
            expect(res.success).toBe(true); expect(res.dryRun).toBe(true); expect(res.findingCode).toBe(FINDING_CODES.S3_PUBLIC_ACCESS); expect(res.action).toBe('AUTO_REMEDIATE'); expect(res.targetResource).toBe('my-corp-bucket');
        });
        it('should execute dry-run for SG open SSH with structured response', async () => {
            ...
            expect(res.success).toBe(true); expect(res.dryRun).toBe(true); expect(res.findingCode).toBe(FINDING_CODES.SG_OPEN_SSH_WORLD); expect(res.action).toBe('AUTO_REMEDIATE');
        });
        it('should block and escalate risky un-whitelisted findings to ADVISORY', async () => {
            const res = await runAWSRemediation('aws', mockCredentials, 'IAM Account', 'root', 'Delete all root credentials and rotate primary billing IAM keys', false);
            expect(res.success).toBe(true); expect(res.advisory).toBe(true); expect(res.message).toContain('ADVISORY'); expect(res.message).toContain('manual approval');
        });
    });

    describe('Azure Remediation Blast Radius & Dry-Run Safety', () => {
        const mockAzureCredentials = { tenantId: 'mock-tenant-id', accessKeyId: 'mock-client-id', secretAccessKey: 'mock-client-secret', projectId: 'mock-sub-id' };
        it('should execute dry-run for Azure App Service HTTPS enforcement', async () => {
            ...
            expect(res.success).toBe(true); expect(res.dryRun).toBe(true); expect(res.findingCode).toBe(FINDING_CODES.AZURE_APPSERVICE_HTTP_ALLOWED);
        });
        it('should execute dry-run for Azure Storage public access', async () => {
            ...
            expect(res.success).toBe(true); expect(res.dryRun).toBe(true); expect(res.findingCode).toBe(FINDING_CODES.AZURE_STORAGE_PUBLIC_BLOB);
        });
        it('should execute dry-run for Azure NSG open inbound rule revocation', async () => {
            ...
            expect(res.success).toBe(true); expect(res.dryRun).toBe(true); expect(res.findingCode).toBe(FINDING_CODES.SG_OPEN_SSH_WORLD);
        });
        it('should execute dry-run for Azure SQL public network access disablement', async () => {
            ...
            expect(res.success).toBe(true); expect(res.dryRun).toBe(true); expect(res.findingCode).toBe(FINDING_CODES.AZURE_SQL_PUBLIC_ACCESS);
        });
        it('should block risky Azure actions outside whitelist and return ADVISORY', async () => {
            ...
            expect(res.success).toBe(true); expect(res.advisory).toBe(true); expect(res.message).toContain('ADVISORY');
        });
    });

    describe('Central Multi-Cloud Remediation Dispatcher', () => {
        it('should route AWS requests with options and canonical candidate action', async () => {
            const code = FINDING_CODES.KMS_KEY_ROTATION_DISABLED;
            const candidate = getRemediationBreakpointDefinition(code);
            const res = await runRemediation('aws', { accessKeyId: 'KEY', secretAccessKey: 'SEC' }, 'KMS Key', 'arn:aws:kms:us-east-1:1111:key/1234', candidate.action, true, { findingCode: code });
            expect(res.success).toBe(true); expect(res.dryRun).toBe(true); expect(res.findingCode).toBe(code);
        });
    });
});
