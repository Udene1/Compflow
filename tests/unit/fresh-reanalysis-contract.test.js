...
describe('fresh remediation reanalysis contract', () => {
  const complete = {
    baselineScanId: 'baseline-scan',
    freshScanId: 'fresh-scan',
  ...
  it('accepts only a distinct provider scan with durable evidence, graph and risk', () => {
    const result = validateFreshReanalysis(complete);
    expect(result.complete).toBe(true);
    expect(result.baselineScanId).toBe('baseline-scan');
    expect(result.freshScanId).toBe('fresh-scan');
  ...
  it('fails closed when the rebuilt graph or deterministic risk is missing', () => {
    expect(() => validateFreshReanalysis({ ...complete, afterPaths: null }))
      .toThrow('REMEDIATION_REANALYSIS_GRAPH_INCOMPLETE');
    expect(() => validateFreshReanalysis({ ...complete, riskAfter: null }))
      .toThrow('REMEDIATION_REANALYSIS_RISK_INCOMPLETE');
  });
});
