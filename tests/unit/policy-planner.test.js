...
const base = {
  id: 'soc2-baseline', version: '1', frameworks: ['soc2'], mode: 'REMEDIATE',
  rules: [
...
function compile() { return compilePolicyPlan({ organizationId: 'org-1', executionId: 'exec-1', policy: base, targets }); }

describe('policy planner', () => {
  it('builds a deterministic dependency graph and self-verifiable hash', () => {
  ...
  it('does not create remediation nodes in audit mode', () => {
    const plan = compilePolicyPlan({ organizationId: 'org-1', executionId: 'exec-4', policy: { ...base, mode: 'AUDIT' }, targets });
    expect(plan.counts.remediations).toBe(0); expect(plan.counts.verifications).toBe(0); expect(plan.counts.approvals).toBe(0);
