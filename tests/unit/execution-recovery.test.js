...
describe('Durable execution heartbeat and stale recovery', () => {
  ...
  beforeAll(async () => {
    ...
    await createExecutionRun({ organizationId, executionId: 'exec_heartbeat_test' });
    await createExecutionRun({ organizationId, executionId: 'exec_recovery_test' });
  });
  ...
  it('does not duplicate terminal recovery or its audit event when recovery runs twice', async () => {
    const executionId = 'exec_recovery_test';
    const before = await pool.query(
      `SELECT count(*)::int AS count FROM execution_events WHERE organization_id=$1 AND execution_id=$2 AND event_type='EXECUTION_LEASE_EXPIRED'`,
      [organizationId, executionId]
    );

    const second = await recoverStaleExecutions({ organizationId, staleAfterSeconds: 30 });
    const after = await pool.query(
      `SELECT count(*)::int AS count FROM execution_events WHERE organization_id=$1 AND execution_id=$2 AND event_type='EXECUTION_LEASE_EXPIRED'`,
      [organizationId, executionId]
    );

    expect(second.executions.map(run => run.id)).not.toContain(executionId);
