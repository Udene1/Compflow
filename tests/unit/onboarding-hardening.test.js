...
describe('Onboarding & Cloud Verification Hardening — All Suites', () => {
    ...
    beforeEach(async () => {
        ...
        session = await createSessionToken(
            { id: testUserId, email: 'secops@cloudguard.io', name: 'SecOps Lead' },
            { id: testOrgId, name: 'CloudGuard Corp' },
            ROLES.ADMIN,
            1
        );
    });
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // 1. Cloud Provider Adapters Verification
    // ─────────────────────────────────────────────────────────────────────────
    describe('1. Real Cloud Provider Verification Adapters', () => {
        it('AWS verifier rejects missing or fake credentials', async () => {
            ...
            expect(resNoCreds.verified).toBe(false);
            expect(resNoCreds.errorCode).toBe('CLOUD_AUTHENTICATION_FAILED');

            const resFake = await awsVerifier.verify({
                accessKeyId: 'AKIA_NONEXISTENT_KEY_123',
                secretAccessKey: 'FAKE_SECRET_KEY_abc123'
            });
            expect(resFake.verified).toBe(false);
            expect(resFake.errorCode).toBeDefined();
        });
        ...
        it('Azure verifier rejects invalid or missing credentials', async () => {
            ...
            const resFake = await azureVerifier.verify({
        ...
        it('GCP verifier rejects invalid credentials', async () => {
            ...
            expect(resBadJson.verified).toBe(false);
            expect(resBadJson.errorCode).toBe('CLOUD_AUTHENTICATION_FAILED');
        });

        it('DigitalOcean verifier rejects missing or invalid token', async () => {
            const resNoToken = await digitalOceanVerifier.verify({});
            expect(resNoToken.verified).toBe(false);
            expect(resNoToken.errorCode).toBe('CLOUD_AUTHENTICATION_FAILED');

            // Invalid token hits fetch and returns error
            const resBad = await digitalOceanVerifier.verify({ apiToken: 'dop_v1_invalid_token_123' });
            expect(resBad.verified).toBe(false);
            expect(['CLOUD_AUTHENTICATION_FAILED', 'CLOUD_CONNECTION_TIMEOUT']).toContain(resBad.errorCode);
        }, 10000);
        ...
        it('Universal CloudVerifier dispatcher handles custom adapters and provenance', async () => {
            const customVerifier = new CloudVerifier({
                aws: {
                    verify: async () => ({
                        verified: true,
                        provider: 'aws',
                        accountIdentifier: '987654321098',
                        principal: 'arn:aws:iam::987654321098:role/CompflowAuditRole',
            ...
            expect(res.accountIdentifier).toBe('987654321098');
            expect(res.verificationMethod).toBe('sts:GetCallerIdentity');
            expect(res.principal).toContain('CompflowAuditRole');
        });
    });

    // ─────────────────────────────────────────────────────────────────────────
    // 2. Queue & Scans Execution Invariants
    // ─────────────────────────────────────────────────────────────────────────
        ...
        it('successful cloud verification creates scan, enqueues BullMQ job with references only', async () => {
            ...
            cloudVerifier.registerVerifier('aws', {
                verify: async () => ({
                    verified: true,
                    provider: 'aws',
                    accountIdentifier: '112233445566',
                    principal: 'arn:aws:iam::112233445566:role/CompflowRole',
                    verificationMethod: 'sts:GetCallerIdentity'
                })
            });
            ...
            const connRes = await invokeOnboarding({
                ...
                body: {
                    ...
                    credentials: {
                        accessKeyId: 'AKIA_VALID_MOCK',
                        secretAccessKey: 'SECRET_VALID_MOCK'
                    }
                }
            });
            ...
            // Assert enqueueJob was called
            expect(enqueueSpy).toHaveBeenCalled();
            const jobPayload = enqueueSpy.mock.calls[enqueueSpy.mock.calls.length - 1][0];

            // Critical invariant: Queue payload must contain references ONLY, never credentials!
            expect(jobPayload.scanId).toBe(verifyRes.body.scanId);
            expect(jobPayload.organizationId).toBe(testOrgId);
            expect(jobPayload.connectionId).toBe(connId);
            expect(jobPayload.provider).toBe('aws');
            expect(jobPayload.scanType).toBe('initial_onboarding_scan');

            expect(jobPayload.credentials).toBeUndefined();
            expect(jobPayload.accessKeyId).toBeUndefined();
            expect(jobPayload.secretAccessKey).toBeUndefined();
            expect(jobPayload.token).toBeUndefined();
        ...
        it('failed cloud verification does not enqueue any scan job or create scans record', async () => {
            ...
            cloudVerifier.registerVerifier('aws', {
                verify: async () => ({
                    ...
                    errorMessage: 'Invalid STS credentials'
                })
            });
            ...
            const connRes = await invokeOnboarding({
                ...
                body: {
                    provider: 'aws',
                    credentials: { accessKeyId: 'AKIA_INVALID' }
                }
            });
            ...
            expect(verifyRes.body.errorCode).toBe('CLOUD_AUTHENTICATION_FAILED');

            // Crucial: No job was enqueued
    ...
    describe('3. Decoupled State Machines & Onboarding Prerequisite Validation', () => {
        it('VERIFIED connection does NOT mean scan is completed', async () => {
            cloudVerifier.registerVerifier('azure', {
                verify: async () => ({
                    verified: true,
                    provider: 'azure',
                    accountIdentifier: 'sub_az_123',
                    principal: 'sp_az_client',
            ...
            const connRes = await invokeOnboarding({
                ...
                body: {
                    provider: 'azure',
                    credentials: { tenantId: 't1', clientId: 'c1', clientSecret: 's1' }
                }
            });
        ...
        it('POST /complete blocks skipping prerequisites', async () => {
            // Fresh org session with no objectives and no cloud connection
            const emptyOrgId = 'org_empty_prereq_99';
            const emptySession = await createSessionToken(
                { id: 'usr_empty', email: 'empty@org.io' },
                { id: emptyOrgId, name: 'Empty Org' },
                ROLES.ADMIN,
                1
            );

            // 1. Without organization profile in DB: blocks with organization profile error
            const resNoOrg = await invokeOnboarding({
                method: 'POST',
                url: '/complete',
                sessionToken: emptySession.token
            });
            expect(resNoOrg.statusCode).toBe(400);
            expect(resNoOrg.body.error).toBe('Prerequisite Failed');
            expect(resNoOrg.body.message).toContain('Organization profile must be set');

            // Save organization profile
            await pool.query('INSERT INTO organizations (id, name) VALUES ($1, $2);', [emptyOrgId, 'Empty Org Inc']);

            // 2. Without framework objectives: blocks with framework objective error
            const resNoFw = await invokeOnboarding({
                method: 'POST',
                url: '/complete',
                sessionToken: emptySession.token
            });
            expect(resNoFw.statusCode).toBe(400);
            expect(resNoFw.body.error).toBe('Prerequisite Failed');
            expect(resNoFw.body.message).toContain('compliance framework objective');

            // Save framework objective
            await pool.query(
                'INSERT INTO organization_frameworks (id, org_id, framework_id, status) VALUES ($1, $2, $3, $4);',
                ['fw_obj_99', emptyOrgId, 'soc2', 'selected']
            );

            // 3. Without verified cloud connection: blocks with cloud connection error
            const resNoConn = await invokeOnboarding({
                method: 'POST',
                url: '/complete',
                sessionToken: emptySession.token
            });
            expect(resNoConn.statusCode).toBe(400);
            expect(resNoConn.body.error).toBe('Prerequisite Failed');
            expect(resNoConn.body.message).toContain('verified cloud connection');
        });
    });

    // ─────────────────────────────────────────────────────────────────────────
    // 4. Authoritative Summary & Truthful Metrics
    // ─────────────────────────────────────────────────────────────────────────
    describe('4. Authoritative Summary & Central Truthful Metrics', () => {
        it('returns zero findings and resources when scan is queued/pending (no invented numbers)', async () => {
            const summaryRes = await invokeOnboarding({
                method: 'GET',
                url: '/summary',
                sessionToken: session.token
            });
            ...
            expect(summaryRes.body.scan.resourcesDiscovered).toBe(0);
            expect(summaryRes.body.compliance.evidenceCollected).toBe(0);
            expect(summaryRes.body.compliance.findings).toEqual({
                critical: 0,
                high: 0,
                medium: 0,
                low: 0
        ...
        it('enforces truthful phrasing via formatAssessmentSummary and assertTruthfulPhrasing', () => {
            ...
            expect(label).toBe('87 of 106 selected SOC 2 controls assessed');
            expect(label).not.toContain('% compliant');

            expect(() => assertTruthfulPhrasing('87 of 106 selected SOC 2 controls assessed')).not.toThrow();
            expect(() => assertTruthfulPhrasing('82% SOC 2 compliant')).toThrow(/Deceptive compliance claim/);
            expect(() => assertTruthfulPhrasing('You are 91% compliant')).toThrow();
        });

        it('updates summary when actual scan results and findings are persisted in DB', async () => {
            const summaryOrgId = 'org_summary_test_exclusive';
            const summarySession = await createSessionToken(
                { id: 'usr_summary_lead', email: 'summary@lead.io' },
                { id: summaryOrgId, name: 'Summary Dedicated Corp' },
                ROLES.ADMIN,
                1
            );
            ...
            await pool.query(
                ...
                [summaryOrgId, 'Summary Dedicated Corp']
            );
            await pool.query(
                'INSERT INTO organization_frameworks (id, org_id, framework_id, status) VALUES ($1, $2, $3, $4);',
                ['fw_sum_1', summaryOrgId, 'soc2', 'selected']
            );

            // Insert completed scan record in `scans` table
            const scanId = 'scan_completed_test_101';
            await pool.query(
                `INSERT INTO scans (id, organization_id, connection_id, scan_type, status)
                 VALUES ($1, $2, 'conn_test', 'initial_onboarding_scan', 'COMPLETED');`,
                [scanId, summaryOrgId]
            );

            await pool.query(
                `UPDATE scans 
                 SET resources_discovered = $1, findings_count = $2, evidence_count = $3 
                 WHERE id = $4;`,
                [25, 4, 30, scanId]
            );

            // Insert 4 real findings
            await pool.query(
                `INSERT INTO findings (id, organization_id, scan_id, resource_id, control_id, severity, status, code)
                 VALUES ($1, $2, $3, 's3-bucket-1', 'S3_PUBLIC', 'CRITICAL', 'FAIL', 'S3_PUBLIC_ACCESS');`,
                ['f_1', summaryOrgId, scanId]
            );
            await pool.query(
                `INSERT INTO findings (id, organization_id, scan_id, resource_id, control_id, severity, status, code)
                 VALUES ($1, $2, $3, 'sg-1', 'SG_OPEN_PORTS', 'HIGH', 'FAIL', 'SG_OPEN_SSH');`,
                ['f_2', summaryOrgId, scanId]
            );
            await pool.query(
                `INSERT INTO findings (id, organization_id, scan_id, resource_id, control_id, severity, status, code)
                 VALUES ($1, $2, $3, 'rds-1', 'RDS_PUBLIC', 'MEDIUM', 'FAIL', 'RDS_PUBLIC_IP');`,
                ['f_3', summaryOrgId, scanId]
            );
            await pool.query(
                `INSERT INTO findings (id, organization_id, scan_id, resource_id, control_id, severity, status, code)
                 VALUES ($1, $2, $3, 's3-bucket-2', 'S3_PUBLIC', 'LOW', 'PASS', 'S3_BUCKET_ENCRYPTED');`,
                ['f_4', summaryOrgId, scanId]
            );
            ...
            const summaryRes = await invokeOnboarding({
                method: 'GET',
                url: '/summary',
                sessionToken: summarySession.token
            });

            expect(summaryRes.statusCode).toBe(200);
            expect(summaryRes.body.scan.status).toBe('COMPLETED');
            expect(summaryRes.body.scan.resourcesDiscovered).toBe(25);
            expect(summaryRes.body.compliance.evidenceCollected).toBe(30);
            expect(summaryRes.body.compliance.findings.critical).toBe(1);
            expect(summaryRes.body.compliance.findings.high).toBe(1);
            expect(summaryRes.body.compliance.findings.medium).toBe(1);
            expect(summaryRes.body.compliance.findings.low).toBe(1);
        });
    });

    // ─────────────────────────────────────────────────────────────────────────
    // 5. Authoritative Session Security & Multi-Instance Simulation
    // ─────────────────────────────────────────────────────────────────────────
    describe('5. Authoritative Session Security & Cross-Instance Simulation', () => {
        it('session DB persistence failure prevents authentication success', async () => {
            const querySpy = vi.spyOn(pool, 'query').mockImplementationOnce(async (sql) => {
            ...
            await expect(
                createSessionToken({ id: 'usr_fail', email: 'fail@test.com' }, { id: 'org_fail' })
            ).rejects.toThrow('Session persistence failed: Database connection timeout on write');
        ...
        it('multi-instance simulation: revoked on Instance A is rejected on Instance B with empty cache', async () => {
            ...
            const userSession = await createSessionToken(
                { id: 'usr_cross_instance', email: 'cross@test.com' },
                { id: 'org_cross' },
                ROLES.ADMIN,
                1
            );

            // Token is initially valid on Instance A
            const checkInitial = await validateSessionToken(userSession.token);
            expect(checkInitial.valid).toBe(true);

            // Instance A: User logs out / session is revoked
            await revokeSession(userSession.token);
            expect(await isSessionRevoked(userSession.token)).toBe(true);

            // Simulate Instance B: completely wipe local in-memory cache
            _clearRevocationCache();

            // Instance B validates the token: memory cache is empty, so it must check PostgreSQL
            const checkInstanceB = await validateSessionToken(userSession.token);

            expect(checkInstanceB.valid).toBe(false);
            expect(checkInstanceB.error).toContain('revoked');
        });

        it('session rotation revokes old token and issues fresh valid token', async () => {
            const initialSession = await createSessionToken(
                { id: 'usr_rotate', email: 'rotate@test.com' },
                { id: 'org_rotate' }
            );

            const rotated = await rotateSession(initialSession.token);
            expect(rotated.token).toBeDefined();
            expect(rotated.token).not.toBe(initialSession.token);

            // Old token is revoked
            const oldCheck = await validateSessionToken(initialSession.token);
