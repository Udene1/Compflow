...
describe('Onboarding & Cloud Verification Hardening — All Suites', () => {
    ...
    describe('4. Authoritative Summary & Central Truthful Metrics', () => {
        ...
        it('updates summary when actual scan results and findings are persisted in DB', async () => {
            ...
            await pool.query(`INSERT INTO findings (id, organization_id, scan_id, resource_id, control_id, severity, status, code) VALUES ($1, $2, $3, 'rds-1', 'RDS_PUBLIC', 'MEDIUM', 'FAIL', 'RDS_PUBLIC_IP');`, ['f_3', summaryOrgId, scanId]);
            await pool.query(`INSERT INTO findings (id, organization_id, scan_id, resource_id, control_id, severity, status, code) VALUES ($1, $2, $3, 's3-bucket-2', 'S3_PUBLIC', 'LOW', 'PASS', 'S3_BUCKET_ENCRYPTED');`, ['f_4', summaryOrgId, scanId]);
