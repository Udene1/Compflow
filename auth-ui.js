...
window.AuthUI = (() => {
    ...
    async function init() { ensureAuthGateStyles(); await fetchProviders(); await fetchCurrentUser(); }
    ...
    function messageForApiError(failure) {
        const messages = {
            AUTHENTICATION_REQUIRED: 'Your session is required. Please sign in.', INVALID_SESSION: 'Your session is no longer valid. Please sign in again.', INSUFFICIENT_PERMISSIONS: 'You do not have permission to perform this action.', ENTITLEMENT_REQUIRED: 'Your workspace does not currently have service access.', PILOT_ACCESS_EXPIRED: 'Pilot access has expired. Contact your Compflow administrator.', CLOUD_CONNECTION_NOT_VERIFIED: 'Verify the cloud connection before starting a scan.', QUEUE_UNAVAILABLE: 'The scan queue is temporarily unavailable. Please try again shortly.'
        };
    ...
    async function authFetch(url, options = {}) {
        ...
        if (response.status === 401) {
            ...
            currentUser = null; renderHeaderWidget(); showAuthGate();
            if (window.showToast) window.showToast(messageForApiError(failure));
            return response;
        }
        if (response.status >= 400) {
            const failure = await parseApiError(response);
            // Authentication navigation is intentionally limited to 401. 402/403/409/422/429/5xx remain in-app API failures.
            if (window.showToast) window.showToast(messageForApiError(failure));
        }
        return response;
    }
    ...
    function handleOAuthCallbackParams() {
        ...
        if (authError) {
            ...
            if (window.showToast) window.showToast(messages[authError] || 'Authentication failed. Please try again.'); showAuthGate(); window.history.replaceState({}, document.title, window.location.pathname); return;
        }
    ...
    async function fetchCurrentUser() {
        try {
            ...
            if (res.ok) { const data = await res.json(); currentUser = data.user; hideAuthGate(); updateChecklistStep1(); await evaluateAppMode(); handleOAuthCallbackParams(); }
            else { currentUser = null; showAuthGate(); setMode('activation'); handleOAuthCallbackParams(); }
    ...
    async function pilotLogin() {
        ...
        try {
            ...
            const res = await authFetch(`${API_BASE}/api/pilot/redeem`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code, email, name: email.split('@')[0] }) });
            const data = await res.json().catch(() => ({}));
            if (!res.ok) { if (errorEl) { errorEl.textContent = messageForApiError({ status: res.status, code: data.code || data.errorCode, message: data.message || data.error }); errorEl.style.display = 'block'; } return; }
        ...
        } catch (e) { if (errorEl) { errorEl.textContent = e.message || 'Connection error. Please try again.'; errorEl.style.display = 'block'; } }
    }
    ...
    async function quickDevLogin(role = 'ADMIN') { try { const res = await authFetch(`${API_BASE}/api/auth/dev-login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ role }) }); if (!res.ok) throw new Error('Dev login unavailable'); const data = await res.json(); currentUser = data.user; renderHeaderWidget(); hideAuthGate(); updateStatusStrip(); await evaluateAppMode(); if (window.showToast) window.showToast(`Signed in as ${data.user.name} (${role})`); } catch (e) { if (window.showToast) window.showToast(`Sign-in failed: ${e.message}`); } }
    ...
    function enforceProductionGuards() { const isDevMode = authProviders.devLogin === true; const devRoles = document.getElementById('dev-roles-section'); const prodSection = document.getElementById('prod-account-section'); if (devRoles) devRoles.style.display = isDevMode ? '' : 'none'; if (prodSection) prodSection.style.display = isDevMode ? 'none' : ''; const switchBtn = document.getElementById('btn-switch-workspace'); if (switchBtn) { if (isDevMode) { switchBtn.textContent = 'Switch Role'; switchBtn.onclick = () => { if (window.AuthUI.openAccountModal) window.AuthUI.openAccountModal(); }; } else { switchBtn.textContent = 'Sign out'; switchBtn.onclick = e => logout(e); } } }
    ...
    function updateChecklistStep1() { const desc = document.getElementById('step-auth-desc'); if (!desc || !currentUser) return; const providerMap = { google: 'Google', github: 'GitHub', pilot_code: 'Pilot Code', dev_portal: 'Dev Portal' }; const providerLabel = providerMap[currentUser.provider] || (currentUser.provider || 'SSO'); const displayName = currentUser.name || currentUser.email || 'unknown'; desc.textContent = `Signed in via ${providerLabel} — ${displayName}`; }
    document.addEventListener('DOMContentLoaded', init);
    return { init, authFetch, fetchCurrentUser, getUser, hasPermission, showAuthGate, hideAuthGate, signInWithGoogle, signInWithGitHub, pilotLogin, quickDevLogin, logout, evaluateAppMode, setMode, openAuthModal: openAccountModal, closeAuthModal: closeAccountModal, openAccountModal, closeAccountModal };
})();
