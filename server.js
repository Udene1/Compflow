...
function lambdaAdapter(handler) { return async (req, res) => { try { const event = { httpMethod: req.method, path: req.path, headers: req.headers, queryStringParameters: req.query || null, body: JSON.stringify(req.body || {}), requestContext: {}, authContext: req.user || null }; const result = await handler(event); if (result.headers) for (const [key, val] of Object.entries(result.headers)) res.setHeader(key, val); res.status(result.statusCode || 200); if (typeof result.body === 'string') { try { res.json(JSON.parse(result.body)); } catch { res.send(result.body); } } else if (result.body) res.json(result.body); else res.end(); } catch (err) { console.error('Adapter crash:', err?.message || 'unknown error'); res.status(500).json({ error: 'Internal Server Error' }); } }; }
...
app.post('/api/validate', requireAuth([ROLES.ENGINEER]), validateHandler); app.get('/api/job-status', requireAuth([ROLES.VIEWER]), jobStatusHandler); app.get('/api/job-stream', requireAuth([ROLES.VIEWER]), jobStreamHandler); app.get('/api/execution-graph', requireAuth([ROLES.VIEWER]), executionGraphHandler); app.post('/api/execution-graph', requireAuth([ROLES.ENGINEER]), executionGraphHandler); app.post('/api/monitoring', requireAuth([ROLES.ADMIN]), lambdaAdapter(monitoringHandler)); app.post('/api/jobs', requireAuth([ROLES.ADMIN]), lambdaAdapter(jobsHandler)); app.post('/api/chat', heavyActionLimiter, requireAuth([ROLES.ENGINEER]), lambdaAdapter(chatHandler)); app.all(['/api/remediate', '/api/remediation'], heavyActionLimiter, requireAuth([ROLES.ENGINEER]), remediateHandler); app.all('/api/auditor*', requireAuth([ROLES.AUDITOR]), auditorHandler); app.all('/api/*', (req, res) => res.status(404).json({ error: 'Not Found', message: `API endpoint ${req.method} ${req.path} does not exist.` }));
