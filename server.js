...
import ownerAuthRouter from './api/owner-auth.js';
...
const heavyActionLimiter = rateLimit({ windowMs: 5 * 60 * 1000, limit: 30, standardHeaders: 'draft-7', legacyHeaders: false, message: { error: 'Heavy operation rate limit exceeded. Please wait 5 minutes before retrying.' } });
...
const ownerAuthLimiter = rateLimit({ windowMs: 15 * 60 * 1000, limit: 10, standardHeaders: 'draft-7', legacyHeaders: false, message: { error: 'Too many owner authentication attempts. Please wait before trying again.' } });
...
app.post('/api/auth/pilot-login', authLimiter, (req, res) => res.status(410).json({ error: 'LEGACY_PILOT_CODE_DISABLED', code: 'LEGACY_PILOT_CODE_DISABLED', message: 'Global pilot codes have been retired. Use your unique pilot invitation link or code.' }));
...
app.use('/api/owner-auth', ownerAuthLimiter, ownerAuthRouter);
