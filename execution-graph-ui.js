(() => {
  ...
  const stateClass = (status) => String(status || 'PENDING').toLowerCase();
  const label = (node) => node.label || node.logical_key || node.node_type;

  function message(text, error = false) {
    const el = $('graph-message');
    el.style.display = text ? 'block' : 'none';
    el.textContent = text || '';
    el.style.borderColor = error ? 'rgba(239,68,68,.45)' : 'rgba(99,102,241,.35)';
    el.style.color = error ? 'var(--danger)' : 'var(--text)';
  }
  ...
  function renderGraph(data) {
    ...
    levels.forEach((level, x) => level.forEach((node, y) => positions.set(node.id, { x: 80 + x * 260, y: 50 + y * 105 })));
    const width = Math.max(900, levels.length * 260 + 120);
    const height = Math.max(430, Math.max(...levels.map(l => l.length), 1) * 105 + 90);
    ...
    const marker = document.createElementNS(ns, 'marker'); marker.setAttribute('id','arrow'); marker.setAttribute('markerWidth','8'); marker.setAttribute('markerHeight','8'); marker.setAttribute('refX','7'); marker.setAttribute('refY','4'); marker.setAttribute('orient','auto');
    const path = document.createElementNS(ns, 'path'); path.setAttribute('d','M0,0 L8,4 L0,8 z'); path.setAttribute('fill','rgba(148,163,184,.65)'); marker.appendChild(path); defs.appendChild(marker); svg.appendChild(defs);
  ...
  async function load() {
    const executionId=$('execution-id').value.trim(); if(!executionId){message('Enter an execution ID to inspect.');return;}
    try { const response=await fetch(`${API}/api/execution-graph?executionId=${encodeURIComponent(executionId)}`,{credentials:'include'}); const body=await response.json().catch(()=>({})); if(!response.ok) throw new Error(body.error || `Request failed (${response.status})`); message(''); renderGraph(body); $('live-indicator').textContent='● Live refresh every 5s'; } catch(error) { message(error.message,true); $('live-indicator').textContent='Live refresh unavailable'; }
  }

  async function resume() {
    const executionId=$('execution-id').value.trim(); if(!executionId)return;
    $('resume-execution').disabled=true;
    try { const response=await fetch(`${API}/api/execution-graph`,{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({executionId})}); const body=await response.json().catch(()=>({})); if(!response.ok) throw new Error(body.error || `Resume failed (${response.status})`); message(`Resume queued: ${body.jobId || 'accepted'}`); await load(); } catch(error){message(error.message,true);} finally{$('resume-execution').disabled=false;}
  }

  $('refresh-graph').addEventListener('click', load);
  $('resume-execution').addEventListener('click', resume);
  $('execution-id').addEventListener('change', load);
