...
function recordsFromEvents(rows) {
  ...
  for (const event of rows) {
    const record = event.payload?.remediation; if (!record?.id) continue;
...
export async function verifyRemediation({ organizationId, executionId, remediationId, actorId, idempotencyKey = null }) {
  ...
  const source = await pool.query(`SELECT id,collected_at,evidence_hash,evidence FROM execution_evidence_records WHERE organization_id=$1 AND execution_id=$2 AND resource_id=$3 AND collected_at > $4 ORDER BY collected_at DESC LIMIT 1`, [organizationId, executionId, current.resourceId, current.appliedAt]);
  if (!source.rows[0]) {
