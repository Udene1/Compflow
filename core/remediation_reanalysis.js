...
async function createReanalysisScan({ organizationId, connection, resources, fallbackCode, fallbackControlId, targetResourceId }) {
  ...
  try {
    ...
    const failures = resources.filter(resource => clean(resource?.severity || resource?.status).toLowerCase() !== 'pass');
    let findingCount = 0;
...
export async function runRemediationReanalysis({ organizationId, executionId, remediationId, findingId, evidenceId = null, beforePaths = [], riskBefore = null, actorId = null, req = null } = {}) {
  ...
  const node = await upsertGraphNode({ organizationId, executionId, nodeType: 'REMEDIATION_REANALYSIS', logicalKey: remediationId, status: 'PENDING', label: `Fresh remediation reanalysis — ${remediationId}`, metadata: { provider: definition.id, connectionId: connection.id, remediationId, baselineScanId } }); const attempt = await startNodeAttempt({ organizationId, executionId, nodeId: node.id, metadata: { provider: definition.id, remediationId, baselineScanId } }); await appendExecutionEvent({ organizationId, executionId, nodeId: node.id, attemptId: attempt.id, eventType: START_EVENT, actorType: 'SYSTEM', actorId, result: 'started', payload: { remediationId, provider: definition.id, baselineScanId } });
  ...
  try {
    ...
    await pool.query(`UPDATE scans SET status='COMPLETED',completed_at=NOW(),evidence_count=$1 WHERE organization_id=$2 AND id=$3 AND status='RUNNING'`, [1, organizationId, fresh.scanId]);
    const scanCheck = await pool.query('SELECT status,resources_discovered,findings_count,evidence_count FROM scans WHERE organization_id=$1 AND id=$2 LIMIT 1', [organizationId, fresh.scanId]); if (scanCheck.rows[0]?.status !== 'COMPLETED' || Number(scanCheck.rows[0]?.resources_discovered) < 1 || Number(scanCheck.rows[0]?.findings_count) !== fresh.findingCount || Number(scanCheck.rows[0]?.evidence_count) < 1) throw new Error('REMEDIATION_REANALYSIS_SCAN_INCOMPLETE');
    ...
    const completeness = validateFreshReanalysis({ baselineScanId, freshScanId: fresh.scanId, resourcesObserved: result.resources.length, evidenceId: evidenceRow.id, evidenceHash: evidenceRow.evidence_hash, afterPaths: persistedAfterPaths, riskAfter }); const beforeComparable = baselinePaths.map(path => ({ ...path, comparisonKey: stableExposurePathKey(path) })); const afterComparable = persistedAfterPaths.map(path => ({ ...path, comparisonKey: stableExposurePathKey(path) })); const diff = compareExposurePathSets({ before: beforeComparable, after: afterComparable, afterComplete: completeness.complete }); const riskDelta = Number(riskAfter.score) - Number(baselineRisk.score); const claimSafe = completeness.complete && (diff.removed.length > 0 || riskDelta < 0);
    const payload = { contractVersion: completeness.version, remediationId: clean(remediationId, 128), findingId: clean(findingId, 128), evidenceId: evidenceRow.id, evidenceHash: evidenceRow.evidence_hash, freshScanId: fresh.scanId, baselineScanId, baselineEvidenceId: evidenceId || null, beforePaths: beforeComparable, afterPaths: afterComparable, afterComplete: completeness.complete, riskBefore: baselineRisk, riskAfter, pathRemoved: diff.removed.length, pathAdded: diff.added.length, pathUnchanged: diff.unchanged.length, riskDelta, claimSafe, provider: definition.id, resourcesObserved: result.resources.length, freshFindingCount: fresh.findingCount, completedAt: new Date().toISOString() };
