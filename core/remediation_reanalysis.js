...
async function createReanalysisScan({ organizationId, connection, resources }) { const scanId = `reanalysis_scan_${crypto.randomUUID()}`; const client = await pool.connect(); try { await client.query('BEGIN'); await client.query(`INSERT INTO scans (id,organization_id,connection_id,scan_type,status,started_at,completed_at,resources_discovered,findings_count,evidence_count) VALUES ($1,$2,$3,'remediation_reanalysis','COMPLETED',NOW(),NOW(),$4,$5,1)`, [scanId, organizationId, connection.id, resources.length, resources.filter(resource => clean(resource?.severity || resource?.status).toLowerCase() !== 'pass').length]); for (const resource of resources.slice(0, MAX_FINDINGS)) { if (clean(resource?.severity || resource?.status).toLowerCase() === 'pass') continue; const id = `finding_${crypto.randomUUID()}`; await client.query('INSERT INTO findings (id,organization_id,scan_id,resource_id,control_id,severity,status,code) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)', [id, organizationId, scanId, resourceId(resource), controlId(resource), severity(resource), 'FAIL', findingCode(resource)]); } await client.query('COMMIT'); return scanId; } catch (error) { await client.query('ROLLBACK').catch(() => {}); throw error; } finally { client.release(); } }
...
export async function runRemediationReanalysis({ organizationId, executionId, remediationId, findingId, evidenceId = null, beforePaths = [], riskBefore = null, actorId = null, req = null } = {}) {
  ...
  try {
    ...
    const freshScanId = await createReanalysisScan({ organizationId, connection, resources: result.resources });
    ...
    await pool.query(`UPDATE execution_runs SET metadata = metadata || $1::jsonb, updated_at=NOW() WHERE organization_id=$2 AND id=$3`, [JSON.stringify({ baselineScanId, scanId: freshScanId, remediationReanalysisScanId: freshScanId }), organizationId, executionId]); switchedToFreshScan = true;
    ...
    if (switchedToFreshScan) {
      await pool.query('UPDATE execution_runs SET metadata=$1::jsonb,updated_at=NOW() WHERE organization_id=$2 AND id=$3', [JSON.stringify(previousMetadata), organizationId, executionId]).catch(() => {});
      await analyzeExecutionExposurePaths({ organizationId, executionId }).catch(() => {});
    }
