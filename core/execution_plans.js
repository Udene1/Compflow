...
export async function persistExecutionPlan({ organizationId, executionId, plan, client = null } = {}) {
  ...
  const existing = await db.query('SELECT * FROM execution_plans WHERE organization_id=$1 AND execution_id=$2 FOR UPDATE', [organizationId, executionId]);
  if (existing.rows[0] && existing.rows[0].plan_hash !== plan.planHash) throw new Error('EXECUTION_PLAN_IMMUTABLE');
  if (existing.rows[0]) return existing.rows[0];
  const result = await db.query(`INSERT INTO execution_plans (id,organization_id,execution_id,policy_id,policy_version,plan_version,plan_hash,plan) VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb) ON CONFLICT (organization_id,execution_id) DO NOTHING RETURNING *`, [id, organizationId, executionId, plan.policy.id, plan.policy.version, plan.planVersion, plan.planHash, canonicalPlan(plan)]);
  if (result.rows[0]) return result.rows[0];
  const concurrent = await db.query('SELECT * FROM execution_plans WHERE organization_id=$1 AND execution_id=$2', [organizationId, executionId]);
  if (!concurrent.rows[0]) throw new Error('PLAN_PERSISTENCE_FAILED');
  if (concurrent.rows[0].plan_hash !== plan.planHash) throw new Error('EXECUTION_PLAN_IMMUTABLE');
  return concurrent.rows[0];
}
export async function getExecutionPlan({ organizationId, executionId, client = null } = {}) { await ensureSchema(); assertId(organizationId, 'PLAN_ORGANIZATION_REQUIRED'); assertId(executionId, 'PLAN_EXECUTION_REQUIRED'); const db = client || pool; const result = await db.query('SELECT * FROM execution_plans WHERE organization_id=$1 AND execution_id=$2', [organizationId, executionId]); return result.rows[0] || null; }
export async function materializeExecutionPlan({ organizationId, executionId, plan, client = null } = {}) {
  validatePolicyPlan(plan); await ensureExecutionGraph(); const ownTransaction = !client; const tx = client || await pool.connect();
  try {
  ...
  } catch (error) { if (ownTransaction) await tx.query('ROLLBACK').catch(() => {}); throw error; } finally { if (ownTransaction) tx.release(); }
}
