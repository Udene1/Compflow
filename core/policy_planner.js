...
function knownControl(controlId, frameworks) { if (Object.prototype.hasOwnProperty.call(ControlMatrix, controlId)) return frameworks.some(framework => Array.isArray(ControlMatrix[controlId]?.[framework]) && ControlMatrix[controlId][framework].length > 0); return Object.values(ControlMatrix).some(mapping => frameworks.some(framework => Array.isArray(mapping?.[framework]) && mapping[framework].includes(controlId))); }
function normalizeRule(rule, index, frameworks) { if (!rule || typeof rule !== 'object' || Array.isArray(rule)) throw new Error(`POLICY_RULE_INVALID:${index}`); const id = assertString(rule.id, `POLICY_RULE_ID_INVALID:${index}`); const controlId = assertString(rule.controlId, `POLICY_RULE_CONTROL_INVALID:${index}`); if (!knownControl(controlId, frameworks)) throw new Error(`POLICY_CONTROL_UNKNOWN:${controlId}`); const action = rule.action || 'EVALUATE'; if (!ACTIONS.has(action)) throw new Error(`POLICY_RULE_ACTION_INVALID:${index}`); return { id, controlId, action, requiresApproval: action === 'REMEDIATE' ? rule.requiresApproval !== false : false }; }
function normalizeTargets(targets) { if (!Array.isArray(targets) || targets.length < 1 || targets.length > MAX_TARGETS) throw new Error('POLICY_TARGETS_INVALID'); return targets.map((target, index) => { if (!target || typeof target !== 'object' || Array.isArray(target)) throw new Error(`POLICY_TARGET_INVALID:${index}`); const provider = assertString(target.provider, `POLICY_TARGET_PROVIDER_INVALID:${index}`); let canonicalProvider; try { canonicalProvider = getProvider(provider).id; } catch { throw new Error(`POLICY_TARGET_PROVIDER_INVALID:${index}`); } return { connectionId: assertString(target.connectionId, `POLICY_TARGET_CONNECTION_INVALID:${index}`), provider: canonicalProvider, resourceId: target.resourceId == null ? null : assertString(target.resourceId, `POLICY_TARGET_RESOURCE_INVALID:${index}`, 256) }; }); }
...
export function compilePolicyPlan({ organizationId, executionId, policy, targets }) {
  ...
  const rules = policy.rules.map((rule, index) => normalizeRule(rule, index, frameworks)); const normalizedTargets = normalizeTargets(targets);
  ...
  for (const target of normalizedTargets) for (const rule of rules) {
    ...
    if (mode === 'REMEDIATE' && rule.action === 'REMEDIATE') {
      ...
      const remediation = node({ organizationId, executionId, nodeType: 'REMEDIATION', logicalKey: `${evidenceKey}:remediate:${rule.id}`, label: `Remediate ${rule.controlId}`, metadata: { ruleId: rule.id, controlId: rule.controlId, frameworks, connectionId: target.connectionId, provider: target.provider, resourceId: target.resourceId, requiresApproval: rule.requiresApproval } }); nodes.push(remediation); edges.push(edge(executionId, dependency.id, remediation.id, { reason: 'approved-remediation' }));
      const verification = node({ organizationId, executionId, nodeType: 'VERIFICATION', logicalKey: `${evidenceKey}:verify:${rule.id}`, label: `Verify ${rule.controlId}`, metadata: { ruleId: rule.id, controlId: rule.controlId, frameworks, connectionId: target.connectionId, provider: target.provider, resourceId: target.resourceId } }); nodes.push(verification); edges.push(edge(executionId, remediation.id, verification.id, { reason: 'post-remediation-verification' }));
    }
  }
