...
export async function recoverStaleNodeAttempts({ organizationId, executionId = null, staleAfterSeconds = 300 } = {}) {
  ...
  const result = await pool.query(
    `UPDATE execution_attempts SET status='FAILED',finished_at=NOW(),error_code='STALE_ATTEMPT',error_message='Execution worker heartbeat expired'
     WHERE organization_id=$1 AND status='RUNNING' AND heartbeat_at < NOW() - ($2 * INTERVAL '1 second') ${executionFilter} RETURNING *`,
    params
  );
  for (const attempt of result.rows) {
    await pool.query(
      `UPDATE execution_graph_nodes SET status='FAILED',updated_at=NOW() WHERE id=$1 AND organization_id=$2 AND execution_id=$3 AND status='RUNNING'`,
      [attempt.node_id, attempt.organization_id, attempt.execution_id]
    );
  }
  return result.rows;
}
...
export async function finishNodeAttempt({ attemptId, status, errorCode = null, errorMessage = null, metadata = {} }) {
  ...
  const result = await pool.query(
    `UPDATE execution_attempts SET status=$1,finished_at=NOW(),heartbeat_at=NOW(),error_code=$2,error_message=$3,metadata=metadata||$4::jsonb WHERE id=$5 AND status='RUNNING' RETURNING *`,
    [status, errorCode, cleanErrorMessage(errorMessage), JSON.stringify(metadata), attemptId]
  );
  ...
  if (current.rows[0]) {
    try { assertNodeTransition(current.rows[0].status, nodeStatus); } catch { return result.rows[0]; }
    await pool.query(`UPDATE execution_graph_nodes SET status=$1,updated_at=NOW() WHERE id=$2 AND organization_id=$3 AND execution_id=$4`, [nodeStatus, result.rows[0].node_id, result.rows[0].organization_id, result.rows[0].execution_id]);
  }
  return result.rows[0];
}
...
export async function getExecutionGraph(organizationId, executionId) {
  ...
  return {
    executionId, organizationId, nodes: nodes.rows, edges: edges.rows, attempts: attempts.rows,
    timeline: attempts.rows.map(a => ({ id: a.id, nodeId: a.node_id, attemptNumber: a.attempt_number, status: a.status, startedAt: a.started_at, finishedAt: a.finished_at, heartbeatAt: a.heartbeat_at, errorCode: a.error_code }))
  };
}
...
export async function getResumableNodes(organizationId, executionId) {
  ...
  return graph.nodes
    .filter(node => retryable.has(node.status))
    .filter(node => graph.edges.filter(e => e.to_node_id === node.id && e.edge_type === 'DEPENDS_ON').every(e => {
      const dependency = graph.nodes.find(n => n.id === e.from_node_id);
...
export async function claimResumableNode({ organizationId, executionId, nodeId, metadata = {} }) {
  const nodes = await getResumableNodes(organizationId, executionId);
  if (!nodes.some(n => n.id === nodeId)) throw new Error('NODE_NOT_RESUMABLE');
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1,0))', [nodeId]);
...
export async function resumeExecution(organizationId, executionId) {
  const nodes = await getResumableNodes(organizationId, executionId);
  return { organizationId, executionId, resumableNodeIds: nodes.map(n => n.id), nodes };
}
