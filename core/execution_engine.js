...
CREATE INDEX IF NOT EXISTS execution_graph_nodes_execution_idx
  ON execution_graph_nodes (organization_id, execution_id);
...
CREATE TABLE IF NOT EXISTS execution_graph_edges (
  ...
  metadata JSONB NOT NULL DEFAULT '{}'::jsonb,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (execution_id, from_node_id, to_node_id, edge_type)
);
CREATE INDEX IF NOT EXISTS execution_graph_edges_execution_idx
  ON execution_graph_edges (organization_id, execution_id);
...
CREATE INDEX IF NOT EXISTS execution_attempts_execution_idx
  ON execution_attempts (organization_id, execution_id, started_at);
`;
...
async function ensureSchema() {
  if (!schemaPromise) {
    schemaPromise = pool.query(SCHEMA_SQL).catch((error) => {
      schemaPromise = null;
      throw error;
    });
  }
...
export function stableUsageId(organizationId, executionId, nodeType, logicalKey) {
  const material = [organizationId, executionId, nodeType, logicalKey]
    .map(value => String(value ?? '').trim())
    .join(':');
  return `node_${crypto.createHash('sha256').update(material).digest('hex').slice(0, 32)}`;
}

export function stableEdgeId(executionId, fromNodeId, toNodeId, edgeType = 'DEPENDS_ON') {
  const material = [executionId, fromNodeId, toNodeId, edgeType].join(':');
  return `edge_${crypto.createHash('sha256').update(material).digest('hex').slice(0, 32)}`;
}

export async function ensureExecutionGraph() {
  await ensureSchema();
}

export async function upsertGraphNode({ organizationId, executionId, nodeType, logicalKey, status = 'PENDING', label = null, metadata = {} }) {
  await ensureSchema();
  if (!organizationId || !executionId || !nodeType || !logicalKey) {
    throw new Error('GRAPH_NODE_INPUT_INVALID');
  }
  const id = stableUsageId(organizationId, executionId, nodeType, logicalKey);
  const result = await pool.query(`
    INSERT INTO execution_graph_nodes
      (id, organization_id, execution_id, node_type, logical_key, status, label, metadata)
    VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb)
    ON CONFLICT (organization_id, execution_id, node_type, logical_key)
...
export async function addDependencyEdge({ organizationId, executionId, fromNodeId, toNodeId, edgeType = 'DEPENDS_ON', metadata = {} }) {
  await ensureSchema();
  if (!organizationId || !executionId || !fromNodeId || !toNodeId || fromNodeId === toNodeId) {
    throw new Error('GRAPH_EDGE_INPUT_INVALID');
  }
  const id = stableEdgeId(executionId, fromNodeId, toNodeId, edgeType);
  const result = await pool.query(`
    INSERT INTO execution_graph_edges
      (id, organization_id, execution_id, from_node_id, to_node_id, edge_type, metadata)
    VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb)
    ON CONFLICT (execution_id, from_node_id, to_node_id, edge_type)
    DO UPDATE SET metadata = execution_graph_edges.metadata || EXCLUDED.metadata
    RETURNING *
...
export async function finishNodeAttempt({ attemptId, status, errorCode = null, errorMessage = null, metadata = {} }) {
  await ensureSchema();
  const allowed = new Set(['SUCCEEDED', 'FAILED', 'CANCELLED', 'SKIPPED']);
  if (!allowed.has(status)) throw new Error('ATTEMPT_STATUS_INVALID');
  const result = await pool.query(`
    UPDATE execution_attempts
    SET status=$1, finished_at=NOW(), error_code=$2, error_message=$3, metadata=metadata || $4::jsonb
    WHERE id=$5
    RETURNING *
  `, [status, errorCode, errorMessage, JSON.stringify(metadata), attemptId]);
  if (!result.rows[0]) throw new Error('ATTEMPT_NOT_FOUND');
  const nodeStatus = status === 'SUCCEEDED' ? 'SUCCEEDED' : status === 'SKIPPED' ? 'SKIPPED' : status === 'CANCELLED' ? 'CANCELLED' : 'FAILED';
  await pool.query(
    'UPDATE execution_graph_nodes SET status=$1, updated_at=NOW() WHERE id=$2',
    [nodeStatus, result.rows[0].node_id]
  );
  return result.rows[0];
}
...
export async function getExecutionGraph(organizationId, executionId) {
  ...
  return {
    ...
    timeline: attempts.rows.map(attempt => ({
      id: attempt.id,
...
export async function resumeExecution(organizationId, executionId) {
  ...
  return { organizationId, executionId, resumableNodeIds: nodes.map(node => node.id), nodes };
}
