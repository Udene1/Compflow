...
function assertNodeTransition(current, next) {
  if (!NODE_STATUS.has(next)) throw new Error('NODE_STATUS_INVALID');
  if (!LEGAL_NODE_TRANSITIONS[current]?.has(next)) throw new Error(`NODE_TRANSITION_INVALID:${current}->${next}`);
}
...
export async function upsertGraphNode({ organizationId, executionId, nodeType, logicalKey, status = 'PENDING', label = null, metadata = {} }) {
  ...
  const result = await pool.query(
    `INSERT INTO execution_graph_nodes (id,organization_id,execution_id,node_type,logical_key,status,label,metadata)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb)
     ON CONFLICT (organization_id,execution_id,node_type,logical_key) DO UPDATE SET
       status=CASE WHEN $6 = execution_graph_nodes.status THEN execution_graph_nodes.status
                   WHEN $6 = 'RUNNING' AND execution_graph_nodes.status IN ('SUCCEEDED','SKIPPED') THEN execution_graph_nodes.status
                   WHEN execution_graph_nodes.status = 'SUCCEEDED' AND $6 IN ('PENDING','FAILED','CANCELLED') THEN execution_graph_nodes.status
                   WHEN execution_graph_nodes.status = 'SKIPPED' AND $6 IN ('PENDING','FAILED','CANCELLED','RUNNING') THEN execution_graph_nodes.status
                   ELSE $6 END,
       label=COALESCE(EXCLUDED.label,execution_graph_nodes.label),
       metadata=execution_graph_nodes.metadata||EXCLUDED.metadata,
       updated_at=NOW()
     RETURNING *`,
    [id, organizationId, executionId, nodeType, logicalKey, status, label, JSON.stringify(metadata)]
  );
...
export async function addDependencyEdge({ organizationId, executionId, fromNodeId, toNodeId, edgeType = 'DEPENDS_ON', metadata = {} }) {
  await ensureSchema();
  if (!organizationId || !executionId || !fromNodeId || !toNodeId || fromNodeId === toNodeId) throw new Error('GRAPH_EDGE_INPUT_INVALID');
  const id = stableEdgeId(executionId, fromNodeId, toNodeId, edgeType);
  const result = await pool.query(
    `INSERT INTO execution_graph_edges (id,organization_id,execution_id,from_node_id,to_node_id,edge_type,metadata)
     VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb)
     ON CONFLICT (execution_id,from_node_id,to_node_id,edge_type) DO UPDATE SET metadata=execution_graph_edges.metadata||EXCLUDED.metadata RETURNING *`,
    [id, organizationId, executionId, fromNodeId, toNodeId, edgeType, JSON.stringify(metadata)]
  );
  return result.rows[0];
}
...
async function createRunningAttempt(client, { organizationId, executionId, nodeId, metadata = {}, resume = false }) {
  ...
  try {
    const result = await client.query(`INSERT INTO execution_attempts (id,organization_id,execution_id,node_id,attempt_number,status,heartbeat_at,metadata) VALUES ($1,$2,$3,$4,$5,'RUNNING',NOW(),$6::jsonb) RETURNING *`, [id, organizationId, executionId, nodeId, attemptNumber, JSON.stringify(resume ? { ...metadata, resume: true } : metadata)]);
    return result.rows[0];
  } catch (error) {
...
export async function startNodeAttempt({ organizationId, executionId, nodeId, metadata = {} }) {
  ...
  try {
    await client.query('BEGIN');
    await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1,0))', [nodeId]);
...
export async function heartbeatNodeAttempt({ attemptId }) {
  await ensureSchema();
  if (!attemptId) throw new Error('ATTEMPT_INPUT_INVALID');
  const result = await pool.query(`UPDATE execution_attempts SET heartbeat_at=NOW() WHERE id=$1 AND status='RUNNING' RETURNING id,heartbeat_at`, [attemptId]);
  if (!result.rows[0]) throw new Error('ATTEMPT_NOT_RUNNING');
  return result.rows[0];
}
...
export async function recoverStaleNodeAttempts({ organizationId, executionId = null, staleAfterSeconds = 300 } = {}) {
  ...
  const params = [organizationId, seconds]; const executionFilter = executionId ? 'AND execution_id=$3' : ''; if (executionId) params.push(executionId);
  const result = await pool.query(`UPDATE execution_attempts SET status='FAILED',finished_at=NOW(),error_code='STALE_ATTEMPT',error_message='Execution worker heartbeat expired' WHERE organization_id=$1 AND status='RUNNING' AND heartbeat_at < NOW() - ($2 * INTERVAL '1 second') ${executionFilter} RETURNING *`, params);
  for (const attempt of result.rows) await pool.query(`UPDATE execution_graph_nodes SET status='FAILED',updated_at=NOW() WHERE id=$1 AND organization_id=$2 AND execution_id=$3 AND status='RUNNING'`, [attempt.node_id, attempt.organization_id, attempt.execution_id]);
  return result.rows;
}

export async function finishNodeAttempt({ attemptId, status, errorCode = null, errorMessage = null, metadata = {} }) {
  await ensureSchema();
  if (!ATTEMPT_STATUS.has(status)) throw new Error('ATTEMPT_STATUS_INVALID');
  const result = await pool.query(`UPDATE execution_attempts SET status=$1,finished_at=NOW(),heartbeat_at=NOW(),error_code=$2,error_message=$3,metadata=metadata||$4::jsonb WHERE id=$5 AND status='RUNNING' RETURNING *`, [status, errorCode, cleanErrorMessage(errorMessage), JSON.stringify(metadata), attemptId]);
  ...
  const current = await pool.query('SELECT status FROM execution_graph_nodes WHERE id=$1 AND organization_id=$2 AND execution_id=$3', [result.rows[0].node_id, result.rows[0].organization_id, result.rows[0].execution_id]);
...
export async function getExecutionGraph(organizationId, executionId) {
  await ensureSchema();
  const [nodes, edges, attempts] = await Promise.all([
  ...
  return { executionId, organizationId, nodes: nodes.rows, edges: edges.rows, attempts: attempts.rows, timeline: attempts.rows.map(a => ({ id: a.id, nodeId: a.node_id, attemptNumber: a.attempt_number, status: a.status, startedAt: a.started_at, finishedAt: a.finished_at, heartbeatAt: a.heartbeat_at, errorCode: a.error_code })) };
}
...
export async function getResumableNodes(organizationId, executionId) {
  ...
  return graph.nodes.filter(node => retryable.has(node.status)).filter(node => graph.edges.filter(e => e.to_node_id === node.id && e.edge_type === 'DEPENDS_ON').every(e => { const dependency = graph.nodes.find(n => n.id === e.from_node_id); return dependency && terminal.has(dependency.status); }));
}
...
export async function claimResumableNode({ organizationId, executionId, nodeId, metadata = {} }) {
  ...
  const client = await pool.connect();
  try {
    await client.query('BEGIN'); await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1,0))', [nodeId]);
    ...
    if (!current.rows[0]) throw new Error('GRAPH_NODE_NOT_FOUND'); if (!['PENDING', 'FAILED', 'CANCELLED'].includes(current.rows[0].status)) throw new Error('NODE_NOT_RESUMABLE');
    const attempt = await createRunningAttempt(client, { organizationId, executionId, nodeId, metadata, resume: true });
    await client.query(`UPDATE execution_graph_nodes SET status='RUNNING',updated_at=NOW() WHERE id=$1 AND organization_id=$2 AND execution_id=$3`, [nodeId, organizationId, executionId]); await client.query('COMMIT'); return attempt;
  } catch (error) { await client.query('ROLLBACK').catch(() => {}); throw error; } finally { client.release(); }
}
