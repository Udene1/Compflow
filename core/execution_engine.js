...
`;
let schemaPromise;
const RANK = { PENDING: 0, RUNNING: 1, FAILED: 2, CANCELLED: 2, SUCCEEDED: 3, SKIPPED: 3 };
...
export async function startNodeAttempt({organizationId,executionId,nodeId,metadata={}}){
  ...
  try{await client.query('BEGIN');await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1,0))',[nodeId]);const node=await client.query('SELECT status FROM execution_graph_nodes WHERE id=$1 AND organization_id=$2 AND execution_id=$3 FOR UPDATE',[nodeId,organizationId,executionId]);if(!node.rows[0]) throw new Error('GRAPH_NODE_NOT_FOUND');if(node.rows[0].status==='SUCCEEDED'||node.rows[0].status==='SKIPPED') throw new Error('NODE_ALREADY_TERMINAL');const latest=await client.query('SELECT COALESCE(MAX(attempt_number),0) AS attempt_number FROM execution_attempts WHERE node_id=$1',[nodeId]);const attemptNumber=Number(latest.rows[0].attempt_number)+1;const id=`attempt_${crypto.randomUUID()}`;const result=await client.query(`INSERT INTO execution_attempts (id,organization_id,execution_id,node_id,attempt_number,status,metadata) VALUES ($1,$2,$3,$4,$5,'RUNNING',$6::jsonb) RETURNING *`,[id,organizationId,executionId,nodeId,attemptNumber,JSON.stringify(metadata)]);await client.query(`UPDATE execution_graph_nodes SET status='RUNNING',updated_at=NOW() WHERE id=$1 AND organization_id=$2 AND execution_id=$3`,[nodeId,organizationId,executionId]);await client.query('COMMIT');return result.rows[0];}catch(error){await client.query('ROLLBACK').catch(()=>{});throw error;}finally{client.release();}
}
export async function finishNodeAttempt({attemptId,status,errorCode=null,errorMessage=null,metadata={}}){
  ...
  const result=await pool.query(`UPDATE execution_attempts SET status=$1,finished_at=NOW(),error_code=$2,error_message=$3,metadata=metadata||$4::jsonb WHERE id=$5 AND status='RUNNING' RETURNING *`,[status,errorCode,cleanErrorMessage(errorMessage),JSON.stringify(metadata),attemptId]);
  ...
  await pool.query(`UPDATE execution_graph_nodes SET status=$1,updated_at=NOW() WHERE id=$2 AND organization_id=$3 AND execution_id=$4 AND (status='RUNNING' OR (status IN ('FAILED','CANCELLED','PENDING') AND $1 IN ('SUCCEEDED','SKIPPED')))`,[nodeStatus,result.rows[0].node_id,result.rows[0].organization_id,result.rows[0].execution_id]);
  return result.rows[0];
}
export async function getExecutionGraph(organizationId,executionId){
  await ensureSchema();const [nodes,edges,attempts]=await Promise.all([pool.query('SELECT * FROM execution_graph_nodes WHERE organization_id=$1 AND execution_id=$2 ORDER BY created_at ASC',[organizationId,executionId]),pool.query('SELECT * FROM execution_graph_edges WHERE organization_id=$1 AND execution_id=$2 ORDER BY created_at ASC',[organizationId,executionId]),pool.query('SELECT * FROM execution_attempts WHERE organization_id=$1 AND execution_id=$2 ORDER BY started_at ASC',[organizationId,executionId])]);
...
export async function claimResumableNode({organizationId,executionId,nodeId,metadata={}}){
  const nodes=await getResumableNodes(organizationId,executionId);if(!nodes.some(n=>n.id===nodeId)) throw new Error('NODE_NOT_RESUMABLE');const client=await pool.connect();
  try{await client.query('BEGIN');await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1,0))',[nodeId]);const current=await client.query('SELECT status FROM execution_graph_nodes WHERE id=$1 AND organization_id=$2 AND execution_id=$3 FOR UPDATE',[nodeId,organizationId,executionId]);if(!current.rows[0]) throw new Error('GRAPH_NODE_NOT_FOUND');if(!['PENDING','FAILED','CANCELLED'].includes(current.rows[0].status)) throw new Error('NODE_NOT_RESUMABLE');const latest=await client.query('SELECT COALESCE(MAX(attempt_number),0) AS attempt_number FROM execution_attempts WHERE node_id=$1',[nodeId]);const attemptNumber=Number(latest.rows[0].attempt_number)+1;const id=`attempt_${crypto.randomUUID()}`;const result=await client.query(`INSERT INTO execution_attempts (id,organization_id,execution_id,node_id,attempt_number,status,metadata) VALUES ($1,$2,$3,$4,$5,'RUNNING',$6::jsonb) RETURNING *`,[id,organizationId,executionId,nodeId,attemptNumber,JSON.stringify({...metadata,resume:true})]);await client.query(`UPDATE execution_graph_nodes SET status='RUNNING',updated_at=NOW() WHERE id=$1 AND organization_id=$2 AND execution_id=$3`,[nodeId,organizationId,executionId]);await client.query('COMMIT');return result.rows[0];}catch(error){await client.query('ROLLBACK').catch(()=>{});throw error;}finally{client.release();}
}
