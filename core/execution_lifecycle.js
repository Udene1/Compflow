...
export async function ensureExecutionLifecycleSchema() {
  await ensureSchema();
}
...
export async function acquireExecutionLease({ organizationId, executionId, workerId, leaseSeconds = 60 } = {}) {
  ...
  try {
    ...
    if (run.status === 'RUNNING' && run.lease_expires_at && new Date(run.lease_expires_at).getTime() > Date.now() && run.lease_owner !== workerId) {
      throw new Error('EXECUTION_ALREADY_LEASED');
    }
