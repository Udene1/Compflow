...
export async function heartbeatExecutionLease({ organizationId, executionId, workerId, leaseToken: token, leaseSeconds = 60 } = {}) {
  ...
  const result = await pool.query(`UPDATE execution_runs SET heartbeat_at=clock_timestamp(), lease_expires_at=clock_timestamp()+($1 * INTERVAL '1 second'), version=version+1, updated_at=clock_timestamp() WHERE id=$2 AND organization_id=$3 AND status='RUNNING' AND lease_owner=$4 AND lease_token=$5 AND lease_expires_at>clock_timestamp() RETURNING id,version,heartbeat_at,lease_expires_at`, [ttl, executionId, organizationId, workerId, token]);
...
export async function finishExecutionRun({ organizationId, executionId, workerId, leaseToken: token, status, errorCode = null, errorMessage = null, metadata = {} } = {}) {
  ...
  try { await client.query('BEGIN'); const current = await client.query('SELECT status FROM execution_runs WHERE id=$1 AND organization_id=$2 AND lease_owner=$3 AND lease_token=$4 AND lease_expires_at>clock_timestamp() FOR UPDATE', [executionId, organizationId, workerId, token]); if (!current.rows[0]) throw new Error('EXECUTION_LEASE_LOST'); assertTransition(current.rows[0].status, status);
    const result = await client.query(`UPDATE execution_runs SET status=$1, finished_at=clock_timestamp(), heartbeat_at=clock_timestamp(), lease_expires_at=NULL, lease_owner=NULL, lease_token=NULL, error_code=$2, error_message=$3, metadata=metadata||$4::jsonb, version=version+1, updated_at=clock_timestamp() WHERE id=$5 AND organization_id=$6 AND lease_owner=$7 AND lease_token=$8 AND lease_expires_at>clock_timestamp() RETURNING *`, [status, errorCode, cleanError(errorMessage), JSON.stringify(metadata), executionId, organizationId, workerId, token]);
...
export async function cancelExecutionRun({ organizationId, executionId, reason = 'Cancelled by operator', actorId = null, idempotencyKey = null } = {}) {
  ...
  try { await client.query('BEGIN'); const result = await client.query(`UPDATE execution_runs SET status='CANCELLED', finished_at=clock_timestamp(), heartbeat_at=clock_timestamp(), lease_expires_at=NULL, lease_owner=NULL, lease_token=NULL, error_code='EXECUTION_CANCELLED', error_message=$3, version=version+1, updated_at=clock_timestamp() WHERE id=$1 AND organization_id=$2 AND status IN ('PENDING','RUNNING','FAILED') RETURNING *`, [executionId, organizationId, cleanError(reason)]);
    ...
    await client.query(`UPDATE execution_attempts SET status='CANCELLED', finished_at=clock_timestamp(), heartbeat_at=clock_timestamp(), error_code='EXECUTION_CANCELLED', error_message=$3 WHERE organization_id=$1 AND execution_id=$2 AND status='RUNNING'`, [organizationId, executionId, cleanError(reason)]).catch(error => { if (error.code !== '42P01') throw error; });
    await client.query(`UPDATE execution_graph_nodes SET status='CANCELLED', updated_at=clock_timestamp() WHERE organization_id=$1 AND execution_id=$2 AND status IN ('PENDING','RUNNING','FAILED')`, [organizationId, executionId]);
    await appendExecutionEvent({ client, organizationId, executionId, eventType: 'EXECUTION_CANCELLED', actorType: 'USER', actorId, result: 'success', payload: { reason: cleanError(reason) }, idempotencyKey }); await client.query('COMMIT'); return result.rows[0];
  } catch (error) { await client.query('ROLLBACK').catch(() => {}); throw error; } finally { client.release(); }
}
export async function recoverExpiredExecutionLeases({ organizationId = null, executionId = null } = {}) {
  await ensureSchema(); const client = await pool.connect(); const params = []; const filters = ["status='RUNNING'", 'lease_expires_at IS NOT NULL', 'lease_expires_at<=clock_timestamp()']; if (organizationId) { params.push(organizationId); filters.push(`organization_id=$${params.length}`); } if (executionId) { params.push(executionId); filters.push(`id=$${params.length}`); }
  try {
    await client.query('BEGIN');
    const runs = await client.query(`SELECT * FROM execution_runs WHERE ${filters.join(' AND ')} FOR UPDATE`, params);
    const recovered = [];
    for (const run of runs.rows) {
      const updated = await client.query(`UPDATE execution_runs SET status='FAILED', finished_at=clock_timestamp(), error_code='STALE_EXECUTION_LEASE', error_message='Execution worker lease expired', lease_owner=NULL, lease_token=NULL, lease_expires_at=NULL, updated_at=clock_timestamp(), version=version+1 WHERE id=$1 AND organization_id=$2 AND status='RUNNING' RETURNING *`, [run.id, run.organization_id]);
      if (!updated.rows[0]) continue;
      const attempts = await client.query(`UPDATE execution_attempts SET status='FAILED', finished_at=clock_timestamp(), heartbeat_at=clock_timestamp(), error_code='STALE_EXECUTION_LEASE', error_message='Execution worker lease expired' WHERE organization_id=$1 AND execution_id=$2 AND status='RUNNING' RETURNING *`, [run.organization_id, run.id]);
      await client.query(`UPDATE execution_graph_nodes SET status='FAILED', updated_at=clock_timestamp() WHERE organization_id=$1 AND execution_id=$2 AND status='RUNNING'`, [run.organization_id, run.id]);
      await appendExecutionEvent({ client, organizationId: run.organization_id, executionId: run.id, eventType: 'EXECUTION_LEASE_EXPIRED', actorType: 'SYSTEM', result: 'failed', payload: { previousStatus: 'RUNNING', errorCode: 'STALE_EXECUTION_LEASE', attemptIds: attempts.rows.map(attempt => attempt.id) } });
      recovered.push({ ...updated.rows[0], recoveredAttempts: attempts.rows });
    }
