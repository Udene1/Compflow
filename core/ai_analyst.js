...
function normalizePath(path) {
  return {
    id: clean(path.id, 128),
    status: clean(path.status, 32),
    severity: clean(path.severity, 32),
    confidence: Number(path.confidence),
    evidenceComplete: Boolean(path.evidence_complete),
    title: clean(path.title, 500),
    summary: clean(path.summary, 1000),
    nodes: Array.isArray(path.nodes) ? path.nodes.slice(0, 20).map(node => ({
      resourceId: clean(node.resource_id, 255),
      nodeType: clean(node.node_type, 64),
      label: clean(node.label, 255),
      findingIds: Array.isArray(node.finding_ids) ? node.finding_ids.slice(0, 50) : [],
      evidenceIds: Array.isArray(node.evidence_ids) ? node.evidence_ids.slice(0, 50) : [],
      metadata: node.metadata && typeof node.metadata === 'object' ? node.metadata : {}
    })) : [],
    edges: Array.isArray(path.edges) ? path.edges.slice(0, 20).map(edge => ({
      relationship: clean(edge.relationship, 128),
      evidenceIds: Array.isArray(edge.evidence_ids) ? edge.evidence_ids.slice(0, 50) : [],
...
export function buildAnalystContext({ findings = [], paths = [], evidence = [] } = {}) {
  ...
  const safeEvidence = evidence.slice(0, 200).map(row => ({
    id: clean(row.id, 128),
    controlId: clean(row.control_id, 128),
    provider: clean(row.provider, 64),
    resourceId: clean(row.resource_id, 255),
    sourceType: clean(row.source_type, 64),
...
function validateAnalysis(value, context) {
  ...
  return {
    ...
    why_it_matters: clean(value.why_it_matters, 4000),
    finding_refs: findingRefs.slice(0, 100),
    path_refs: pathRefs.slice(0, 50),
    evidence_refs: evidenceRefs.slice(0, 200),
    affected_resources: Array.isArray(value.affected_resources) ? value.affected_resources.slice(0, 100).map(item => clean(item, 255)) : [],
    uncertainty: clean(value.uncertainty, 3000),
    recommended_breakpoints: Array.isArray(value.recommended_breakpoints) ? value.recommended_breakpoints.slice(0, 20).map(item => ({
      finding_id: clean(item.finding_id, 128),
...
${JSON.stringify(safeContext)}`;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt += 1) {
    try {
      ...
      return { analysis: validateAnalysis(parsed, safeContext), model: MODEL, promptVersion: PROMPT_VERSION, sourceHash };
    } catch (error) {
      lastError = error;
...
export async function analyzeExecutionSecurity({ organizationId, executionId, actorId = null, idempotencyKey = null } = {}) {
  ...
  const findings = scanId
    ? await pool.query('SELECT id,resource_id,control_id,severity,status,code FROM findings WHERE organization_id=$1 AND scan_id=$2 ORDER BY created_at ASC LIMIT $3', [organizationId, scanId, MAX_FINDINGS])
    : { rows: [] };
  const context = buildAnalystContext({ findings: findings.rows, paths: paths.rows, evidence: evidence.rows });
...
export async function getLatestSecurityAnalysis({ organizationId, executionId } = {}) {
  if (!organizationId || !executionId) throw new Error('AI_ANALYST_INPUT_INVALID');
  const result = await pool.query(`SELECT * FROM execution_events WHERE organization_id=$1 AND execution_id=$2 AND event_type='SECURITY_ANALYSIS_COMPLETED' ORDER BY sequence DESC LIMIT 1`, [organizationId, executionId]);
