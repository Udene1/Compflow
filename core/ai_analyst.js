...
const MAX_PATHS = 50;
const PROMPT_VERSION = 'security-analyst-v4';
...
export function buildAnalystContext({ findings = [], paths = [], evidence = [], risk = null, breakpoints = null, remediationProofs = [] } = {}) {
  ...
  return {
    findings: safeFindings, paths: safePaths, evidence: safeEvidence,
    ...
    remediationProofs: remediationProofs.slice(0, 50).map(item => ({ remediationId: clean(item.remediationId, 128), findingId: clean(item.findingId, 128), baselineScanId: clean(item.baselineScanId, 128), freshScanId: clean(item.freshScanId, 128), evidenceId: clean(item.evidenceId, 128), evidenceHash: clean(item.evidenceHash, 128), afterComplete: item.afterComplete === true, riskBefore: Number(item.riskBefore?.score), riskAfter: Number(item.riskAfter?.score), pathRemoved: Number(item.pathRemoved || 0), pathAdded: Number(item.pathAdded || 0), claimSafe: item.claimSafe === true }))
  };
}
...
function validateAnalysis(value, context) {
  ...
  const allowedFindingIds = new Set(context.findings.map(item => item.id)); const allowedEvidenceIds = new Set(context.evidence.map(item => item.id)); const allowedPathIds = new Set(context.paths.map(item => item.id)); const allowedBreakpointFindingIds = new Set((context.remediationBreakpoints || []).map(item => item.findingId));
  const evidenceRefs = Array.isArray(value.evidence_refs) ? value.evidence_refs : []; const findingRefs = Array.isArray(value.finding_refs) ? value.finding_refs : []; const pathRefs = Array.isArray(value.path_refs) ? value.path_refs : [];
  if (findingRefs.some(id => !allowedFindingIds.has(id))) throw new Error('AI_ANALYST_UNGROUNDED_FINDING_REF'); if (evidenceRefs.some(id => !allowedEvidenceIds.has(id))) throw new Error('AI_ANALYST_UNGROUNDED_EVIDENCE_REF'); if (pathRefs.some(id => !allowedPathIds.has(id))) throw new Error('AI_ANALYST_UNGROUNDED_PATH_REF'); if (value.compromise_confirmed !== false) throw new Error('AI_ANALYST_COMPROMISE_CLAIM_FORBIDDEN');
...
export async function analyzeSecurityContext(context) {
  ...
  const safeContext = buildAnalystContext(context); const sourceHash = sha256(JSON.stringify(safeContext)); const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY); const model = genAI.getGenerativeModel({ model: MODEL });
  const prompt = `You are Compflow's security analyst. Explain security/compliance state using ONLY supplied deterministic facts. Risk, exposure paths, remediation breakpoints, and remediation proofs are authoritative. Do not recalculate, override, or invent them. Do not invent relationships, findings, resources, evidence, controls, exploits, or compromise. Do not infer compromise from any path. Every reference must use an ID present in the input. If evidence is incomplete, say so. A remediation proof is authoritative only when claimSafe is true and afterComplete is true; otherwise describe it as incomplete or unproven. Never claim a path or aggregate-risk reduction from a proof that is not claimSafe. Recommended breakpoints may only restate supplied deterministic candidates. Do not claim a change has been executed or verified unless the supplied proof says so.
...
${JSON.stringify(safeContext)}`;
  let lastError;
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt += 1) { try { const result = await model.generateContent(prompt); const parsed = parseModelJson(result.response.text()); return { analysis: validateAnalysis(parsed, safeContext), model: MODEL, promptVersion: PROMPT_VERSION, sourceHash }; } catch (error) { lastError = error; const retryable = error?.status === 429 || error?.status >= 500 || /rate|timeout|temporar/i.test(error?.message || ''); if (!retryable || attempt === MAX_RETRIES) break; await new Promise(resolve => setTimeout(resolve, 500 * 2 ** (attempt - 1))); } }
...
export async function analyzeExecutionSecurity({ organizationId, executionId, actorId = null, idempotencyKey = null } = {}) {
  ...
  const execution = await pool.query('SELECT metadata FROM execution_runs WHERE organization_id=$1 AND id=$2', [organizationId, executionId]); const scanId = execution.rows[0]?.metadata?.scanId || execution.rows[0]?.metadata?.scan_id || null;
  ...
  const proofEvents = await pool.query(`SELECT payload FROM execution_events WHERE organization_id=$1 AND execution_id=$2 AND event_type='REMEDIATION_REANALYSIS_COMPLETED' AND result='completed' ORDER BY sequence DESC LIMIT 50`, [organizationId, executionId]);
  const remediationProofs = proofEvents.rows.map(row => { const p = row.payload || {}; const before = Array.isArray(p.beforePaths) ? p.beforePaths : []; const after = Array.isArray(p.afterPaths) ? afterCount(afterPathsPlaceholder = p.afterPaths) : []; const beforeKeys = new Set(before.map(item => item.comparisonKey)); const afterKeys = new Set(after.map(item => item.comparisonKey)); return { ...p, pathRemoved: [...beforeKeys].filter(key => !afterKeys.has(key)).length, pathAdded: [...afterKeys].filter(key => !beforeKeys.has(key)).length, claimSafe: p.afterComplete === true && Boolean(p.evidenceId && p.evidenceHash) }; });
  ...
  const result = await analyzeSecurityContext(context); const event = await appendExecutionEvent({ organizationId, executionId, actorType: 'SYSTEM', actorId, eventType: 'SECURITY_ANALYSIS_COMPLETED', result: 'success', payload: { ...result, analysis: result.analysis }, idempotencyKey: idempotencyKey ? `security-analysis:${idempotencyKey}` : null });
