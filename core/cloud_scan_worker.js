 ...
 * BullMQ/durable-worker code owns queue and execution fencing; this module owns
 * the actual cloud scan/remediation work and accepts normalized job data only.
 */
export async function processCloudScanJob(jobData = {}) {
    ...
    try {
        ...
        for (const resource of anomalies.slice(0, 10)) {
            await trackProgress('in_progress', 52, 'INSIGHT', `${resource.severity?.toUpperCase()}: ${resource.type} "${resource.name}" — ${resource.issue}`);
        }
        ...
        for (let i = 0; i < anomalies.length; i++) {
            ...
            const progress = 55 + Math.round((i / Math.max(totalAnomalies, 1)) * 25);
            ...
            if (llmDecision.action === 'AUTO_FIX' && client.autoRemediate) {
                ...
                try {
                    ...
                    log.error(`[AGENT] Fix failed for ${anomaly.name}:`, error);
                }
            } else {
                escalatedCount++;
                remediationDetails.push({ name: anomaly.name, status: 'escalated' });
        ...
        const isNonRetryable = message.includes('Authentication Failed') ||
            message.includes('Verification Failed') ||
            message.includes('Missing cloud credentials') ||
