...
import { normalizeRelationshipProvenance, canCountAsVerifiedRelationship } from './exposure_provenance.js';

const MAX_PATHS = 100;
const MAX_DEPTH = 6;
const TERMINAL_RESOURCE_TYPES = new Set(['S3', 'RDS', 'DYNAMODB', 'KEYVAULT', 'SQL', 'SECRETS']);
const EXPOSURE_CODES = new Set(['S3_PUBLIC_ACCESS', 'AZURE_STORAGE_PUBLIC_BLOB', 'RDS_PUBLICLY_ACCESSIBLE', 'AZURE_SQL_PUBLIC_ACCESS', 'SG_OPEN_SSH_WORLD', 'SG_OPEN_RDP_WORLD', 'SG_OPEN_HTTP_WORLD', 'AZURE_NSG_OPEN_INBOUND']);
...
function extractResources(row) {
  ...
  return (Array.isArray(evidence.resources) ? evidence.resources : []).filter(item => item && typeof item === 'object' && clean(item.id || item.resourceId));
}
function extractRelationships(row) {
  const evidence = row.evidence && typeof row.evidence === 'object' ? row.evidence : {};
  const candidates = [];
  if (Array.isArray(evidence.relationships)) {
    candidates.push(...evidence.relationships.map(item => ({ ...item, evidenceId: row.id, observedAt: item.observedAt || row.observed_at || row.collected_at, provider: item.provider || row.provider, sourceRef: item.sourceRef || row.source_ref })));
  }
  for (const resource of extractResources(row)) {
    if (Array.isArray(resource.relationships)) {
      candidates.push(...resource.relationships.map(item => ({ ...item, evidenceId: row.id, observedAt: item.observedAt || row.observed_at || row.collected_at, provider: item.provider || row.provider, sourceRef: item.sourceRef || row.source_ref })));
    }
  }
  return candidates.map(item => {
    const provenance = normalizeRelationshipProvenance({
      provider: item.provider,
      evidenceId: item.evidenceId,
      observedAt: item.observedAt,
      sourceRef: item.sourceRef,
      relationSource: item.relationSource || item.source || 'provider_observation',
...
function serializeFinding(finding) { return { id: clean(finding.id), code: clean(finding.code), severity: severity(finding.severity), control: clean(finding.control || finding.control_id || finding.controlId), issue: clean(finding.issue || finding.title || finding.description), status: clean(finding.status) }; }
...
function buildGraph(evidenceRows, findingRows) {
  ...
  for (const finding of findingRows) {
    const resourceId = clean(finding.resource_id);
    if (!resourceId) continue;
    const list = findingsByResource.get(resourceId) || [];
    list.push(finding);
    findingsByResource.set(resourceId, list);
  }
  for (const row of evidenceRows) {
    for (const item of extractResources(row)) {
  ...
  for (const finding of findingRows) {
    ...
    if (id && !resources.has(id)) resources.set(id, { id, type: 'RESOURCE', label: id, evidenceIds: [] });
  }
...
function findPaths(graph) {
  ...
  function walk(resourceId, nodes, traversedEdges, visited) {
    ...
    if (!resource) return;
    const currentFindings = findingsByResource.get(resourceId) || [];
    const terminal = isTerminal(resource) && (currentFindings.length > 0 || resource.type !== 'RESOURCE');
    const privileged = currentFindings.some(isPrivilege);
    if ((terminal || privileged) && nodes.length > 1) {
      ...
      if (!seen.has(pathKey)) {
        ...
        const verifiedEdges = traversedEdges.filter(edge => canCountAsVerifiedRelationship(edge)).length;
        ...
        const status = traversedEdges.length && traversedEdges.every(edge => canCountAsVerifiedRelationship(edge)) && allNodeEvidence ? 'VERIFIED' : 'POTENTIAL';
        paths.push({
          pathKey,
          status,
          ...
          nodes: nodes.map(node => {
            ...
            return { ...node, findingIds: findings.map(finding => finding.id), findings: findings.map(serializeFinding), evidenceIds: evidenceByResource.get(node.id) || [] };
          }),
    ...
    for (const edge of adjacency.get(resourceId) || []) {
      ...
      if (target) walk(edge.toResourceId, [...nodes, target], [...traversedEdges, edge], new Set([...visited, edge.toResourceId]));
    }
  }
...
export async function analyzeExecutionExposurePaths({ organizationId, executionId, evidenceIds = null } = {}) {
  if (!organizationId || !executionId) throw new Error('EXPOSURE_PATH_INPUT_INVALID');
  await ensureExposurePathSchema();
  const selected = Array.isArray(evidenceIds) ? [...new Set(evidenceIds.map(id => clean(id, 128)).filter(Boolean))] : null;
  if (selected && selected.length === 0) throw new Error('EXPOSURE_PATH_FRESH_EVIDENCE_REQUIRED');
  const evidenceQuery = selected
    ? await pool.query('SELECT * FROM execution_evidence_records WHERE organization_id=$1 AND execution_id=$2 AND id = ANY($3::text[]) ORDER BY collected_at ASC', [organizationId, executionId, selected])
    : await pool.query('SELECT * FROM execution_evidence_records WHERE organization_id=$1 AND execution_id=$2 ORDER BY collected_at ASC', [organizationId, executionId]);
  if (selected && evidenceQuery.rows.length !== selected.length) throw new Error('EXPOSURE_PATH_FRESH_EVIDENCE_INCOMPLETE');
  ...
  const paths = analyzeExposurePaths({ evidenceRows: evidenceQuery.rows, findingRows: findings.rows });
  ...
  try {
    ...
    for (const path of paths) {
      ...
      for (let index = 0; index < path.edges.length; index += 1) {
        ...
        await client.query(`INSERT INTO exposure_path_edges (id,path_id,position,from_node_id,to_node_id,relationship,evidence_ids,metadata) VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb,$8::jsonb)`, [stableId('pathedge', `${pathId}:${index}`), pathId, index, nodeIds[fromIndex], nodeIds[toIndex], edge.relationship, JSON.stringify(edge.evidenceId ? [edge.evidenceId] : []), JSON.stringify({ verified: canCountAsVerifiedRelationship(edge), observed: edge.observed, inferred: edge.inferred, provenanceVersion: edge.version, provider: edge.provider, observedAt: edge.observedAt, sourceRef: edge.sourceRef, confidence: edge.confidence })]);
      }
    }
