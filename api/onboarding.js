...
router.get('/status', async (req, res) => {
    ...
    try {
        ...
        res.json({ onboardingStatus: onboardingState, organization: { id: org.id, name: org.name, domain: org.domain, website: org.website || null, industry: org.industry || null, companySize: org.company_size || null }, frameworks, connections: connRes.rows || [], complianceGuidance: 'Compflow will use these objectives to prioritize relevant controls and evidence.' });
    } catch (err) { log.error(`[ONBOARDING] Error fetching status for org ${orgId}:`, err.message); res.status(500).json({ error: 'Failed to retrieve onboarding status', code: 'ONBOARDING_STATUS_FAILED', message: 'Onboarding status could not be retrieved.' }); }
});

router.post('/organization', async (req, res) => {
    const orgId = req.user.orgId; const { name, website, industry, companySize } = req.body || {};
    if (!name || typeof name !== 'string') return res.status(400).json({ error: 'Validation Error', code: 'ORGANIZATION_NAME_REQUIRED', message: 'Organization name is required.' });
    try {
        ...
        res.json({ success: true, status: next, organization: { id: orgId, name, website, industry, companySize } });
    } catch (err) { log.error('[ONBOARDING] Failed to update organization:', err.message); res.status(500).json({ error: 'Failed to update organization', code: 'ORGANIZATION_UPDATE_FAILED', message: 'Organization details could not be saved.' }); }
});

router.post('/objectives', async (req, res) => {
    const orgId = req.user.orgId; const { frameworks } = req.body || {};
    if (!Array.isArray(frameworks) || frameworks.length === 0) return res.status(400).json({ error: 'Validation Error', code: 'FRAMEWORK_REQUIRED', message: 'Please select at least one compliance framework.' });
    try {
        ...
        await pool.query(`INSERT INTO onboarding_state (org_id, status) VALUES ($1, $2) ON CONFLICT (org_id) DO UPDATE SET status = $2, updated_at = CURRENT_TIMESTAMP;`, [orgId, next]);
        res.json({ success: true, status: next, selectedFrameworks: frameworks, guidance: 'Compflow will use these objectives to prioritize relevant controls and evidence.' });
    } catch (err) { log.error('[ONBOARDING] Failed to save frameworks:', err.message); res.status(500).json({ error: 'Failed to record objectives', code: 'OBJECTIVES_SAVE_FAILED', message: 'Compliance objectives could not be saved.' }); }
});
...
router.post('/cloud-connection', async (req, res) => {
    ...
    if (!provider) return res.status(400).json({ error: 'Validation Error', code: 'PROVIDER_REQUIRED', message: 'Cloud provider is required.' });
    const validProviders = ['aws', 'azure', 'gcp', 'digitalocean', 'hetzner']; const normalizedProvider = String(provider).toLowerCase();
    if (!validProviders.includes(normalizedProvider)) return res.status(400).json({ error: 'Validation Error', code: 'UNSUPPORTED_PROVIDER', message: `Unsupported provider "${provider}".` });
    const connId = 'conn_' + crypto.randomUUID().replace(/-/g, '').substring(0, 16); const credRef = 'sec_ref_' + connId;
    try {
        if (!credentials) return res.status(400).json({ error: 'Validation Error', code: 'CLOUD_CREDENTIALS_REQUIRED', message: 'Cloud credentials are required to create a connection.' });
        ...
        res.status(201).json({ success: true, connectionId: connId, provider: normalizedProvider, status: 'PENDING', displayName: displayName || `${normalizedProvider.toUpperCase()} Connection` });
    } catch (err) { log.error('[ONBOARDING] Cloud connection creation failed:', err.message); res.status(500).json({ error: 'Failed to create cloud connection', code: 'CLOUD_CONNECTION_CREATE_FAILED', message: 'Cloud connection could not be created.' }); }
});
...
router.post('/cloud-connection/:id/verify', async (req, res) => {
    ...
    try {
        ...
        if (!connRes.rows?.length) return res.status(404).json({ error: 'Not Found', code: 'CLOUD_CONNECTION_NOT_FOUND', message: 'Cloud connection not found.' });
        ...
        // A scan is a first-class durable job. Do not enqueue a BullMQ job without its PostgreSQL job record.
        ...
        try {
            ...
            log.info(`[ONBOARDING] Cloud verified. Scan ${scanId} / Job ${jobId} queued for org ${orgId}`);
        } catch (queueErr) {
        ...
        return res.status(finalScanStatus === 'QUEUED' ? 200 : 503).json({ verified: true, status: 'VERIFIED', scanId, jobId, scanStatus: finalScanStatus, onboardingStatus: next, provenance: { accountIdentifier: verifyResult.accountIdentifier, principal: verifyResult.principal, verificationMethod: verifyResult.verificationMethod }, ...(finalScanStatus === 'QUEUED' ? { message: 'Your environment is connected and verified. Compflow has queued your initial compliance scan.' } : { code: 'INITIAL_SCAN_QUEUE_UNAVAILABLE', error: 'Initial scan unavailable', message: 'Your environment is verified, but the initial scan could not be queued. Please retry.' }) });
    } catch (err) {
        ...
        return res.status(500).json({ error: 'Verification failed', code: 'CLOUD_VERIFICATION_FAILED', message: 'Cloud verification could not be completed.' });
    }
});
...
router.post('/complete', async (req, res) => {
    const orgId = req.user.orgId;
    try {
        const orgRes = await pool.query('SELECT id, name FROM organizations WHERE id = $1;', [orgId]); if (!orgRes.rows?.length) return res.status(400).json({ error: 'Prerequisite Failed', code: 'ORGANIZATION_REQUIRED', message: 'Organization profile must be set before completing onboarding.' });
        const fwRes = await pool.query('SELECT framework_id FROM organization_frameworks WHERE org_id = $1;', [orgId]); if (!fwRes.rows?.length) return res.status(400).json({ error: 'Prerequisite Failed', code: 'OBJECTIVES_REQUIRED', message: 'At least one compliance framework objective must be selected before completing onboarding.' });
        const connRes = await pool.query('SELECT id, status FROM cloud_connections WHERE organization_id = $1;', [orgId]); if (!(connRes.rows || []).some(c => c.status === 'VERIFIED')) return res.status(400).json({ error: 'Prerequisite Failed', code: 'VERIFIED_CLOUD_CONNECTION_REQUIRED', message: 'At least one verified cloud connection is required before completing onboarding.' });
        const scanRes = await pool.query('SELECT id, status FROM scans WHERE organization_id = $1;', [orgId]); if (!(scanRes.rows || []).some(s => ['QUEUED', 'RUNNING', 'COMPLETED', 'PARTIAL'].includes(s.status))) return res.status(400).json({ error: 'Prerequisite Failed', code: 'INITIAL_SCAN_REQUIRED', message: 'An initial compliance scan must be queued or completed before completing onboarding.' });
        ...
        res.json({ success: true, status: 'ONBOARDING_COMPLETED', message: 'Onboarding completed successfully. Welcome to Compflow!' });
    } catch (err) { log.error('[ONBOARDING] Failed to complete onboarding:', err.message); res.status(500).json({ error: 'Failed to complete onboarding', code: 'ONBOARDING_COMPLETION_FAILED', message: 'Onboarding could not be completed.' }); }
});
...
router.get('/summary', async (req, res) => {
    ...
    try {
        ...
        const scanRes = await pool.query('SELECT * FROM scans WHERE organization_id = $1 ORDER BY created_at DESC LIMIT 1;'); const latestScan = scanRes.rows?.[0] || null;
        ...
        const frameworkStats = selectedFws.map(fw => { const fwKey = fw.toLowerCase(); const name = getFrameworkDisplayName(fw); const matchingFindings = scanFindings.filter(f => { if (!f.control_id) return false; const matrixEntry = ControlMatrix[f.control_id]; if (!matrixEntry) return true; return Boolean(matrixEntry[fwKey]); }); const controlsNeedingAttention = matchingFindings.filter(f => f.status === 'FAIL').length; const controlsPassing = matchingFindings.filter(f => f.status === 'PASS').length; const controlsAssessed = controlsPassing + controlsNeedingAttention; return { id: fwKey, name, controlsAssessed, controlsPassing, controlsNeedingAttention, assessmentLabel: formatAssessmentSummary({ assessed: controlsAssessed, framework: fw }) }; });
    ...
    } catch (err) { log.error('[ONBOARDING] Failed to retrieve summary:', err.message); return res.status(500).json({ error: 'Failed to retrieve onboarding summary', code: 'ONBOARDING_SUMMARY_FAILED', message: 'Onboarding summary could not be retrieved.' }); }
});
