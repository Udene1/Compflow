...
import { ROLES, createSessionToken, revokeSession } from '../core/auth.js';
...
router.get('/status', async (req, res) => {
    try {
        const result = await pool.query(`SELECT EXISTS (SELECT 1 FROM org_memberships WHERE role = $1 AND status = 'active') AS claimed`, [ROLES.OWNER]);
        const claimed = Boolean(result.rows[0]?.claimed);
        res.json({ passwordLogin: true, ownerBootstrap: !claimed && Boolean(process.env.OWNER_BOOTSTRAP_CODE), claimed, ownerEmail: OWNER_EMAIL });
    } catch (error) {
...
router.post('/bootstrap', async (req, res) => {
    ...
    if (!constantTimeEqual(suppliedCode, bootstrapCode)) return res.status(401).json({ error: 'INVALID_BOOTSTRAP_CODE', message: 'Owner setup could not be authorized.' });
    ...
    try {
        ...
        if (!org) org = (await client.query(`INSERT INTO organizations (id, name, domain, sso_provider, onboarding_status) VALUES ($1,'ComplianceFlow','compflow.icu','native','AUTHENTICATED') RETURNING *`, ['org_compflow_icu'])).rows[0];
        ...
        const session = await createSessionToken({ id: user.id, email: user.email, name: user.name }, { id: org.id, name: org.name }, ROLES.OWNER, 1);
        try {
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (!user || !user.password_hash || !(await verifyPassword(password, user.password_hash))) return res.status(401).json({ error: 'INVALID_CREDENTIALS', message: 'Email or password is incorrect.' });
        const membershipRes = await pool.query(`SELECT m.*, o.name AS org_name FROM org_memberships m JOIN organizations o ON o.id = m.org_id WHERE m.user_id = $1 AND m.status = 'active' ORDER BY CASE m.role WHEN 'OWNER' THEN 5 WHEN 'ADMIN' THEN 4 WHEN 'ENGINEER' THEN 3 WHEN 'AUDITOR' THEN 2 ELSE 1 END DESC LIMIT 1`, [user.id]);
