...
function snapshot(graph, execution, resumableNodeIds, events, recovery = {}) {
  const body = { ...graph, execution, live: true, observedAt: new Date().toISOString(), recovery, resumableNodeIds, events, eventCursor: events.length ? events[events.length - 1].sequence : '0' };
...
async function readState(organizationId, executionId, afterSequence = 0) {
  ...
  return snapshot(graph, execution, resumableNodeIds, events, { executionsRecovered: recoveredExecutions.map(run => run.id), attemptsRecovered: recoveredAttempts.map(attempt => attempt.id) });
}
...
async function streamExecution(res, organizationId, executionId, req) {
  ...
  let afterSequence = Number(req.headers['last-event-sequence'] || 0);
  ...
  const tick = async () => {
    ...
    try {
      const state = await readState(organizationId, executionId, afterSequence);
      if (state.hash !== lastHash) {
        lastHash = state.hash;
        writeSse(res, 'execution', state.body, state.hash);
        for (const event of state.body.events) {
          writeSse(res, 'audit', event, String(event.sequence));
          afterSequence = Math.max(afterSequence, Number(event.sequence) || afterSequence);
        }
      } else {
  ...
  writeSse(res, 'ready', { executionId, intervalMs, afterSequence });
  await tick();
...
async function queueResume({ organizationId, executionId, nodeIds = null, action = 'resume' }) {
  ...
  await recoverStaleNodeAttempts({ organizationId, executionId });
  const plan = await getDependencyAwareResumePlan(organizationId, executionId);
  const requested = nodeIds ? plan.nodes.filter(node => nodeIds.includes(node.id)) : plan.nodes;
  ...
  await enqueueJob({ jobId, scanId, executionId, organizationId, connectionId, provider, scanType: 'resume', resumeNodeIds: requested.map(node => node.id), enqueuedAt: new Date().toISOString(), normalMetadata: JSON.stringify({ control: action }) });
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'resume' || action === 'retry') {
        ...
        return res.status(202).json(await queueResume({ organizationId, executionId, nodeIds, action }));
      }
    ...
    const known = new Map([
      ['EXECUTION_NOT_FOUND', 404], ['EXECUTION_ALREADY_SUCCEEDED', 409], ['EXECUTION_CANCELLED', 409], ['EXECUTION_ALREADY_TERMINAL', 409],
      ['EXECUTION_CANCEL_CONFLICT', 409], ['EXECUTION_NOT_RESUMABLE', 409], ['NODE_NOT_RESUMABLE', 409], ['NO_DEPENDENCY_READY_NODES', 409],
