...
import { runRemediationReanalysis } from '../core/remediation_reanalysis.js';
...
router.post('/executions/:executionId/remediations', async (req, res, next) => {
  try {
    ...
    const candidates=await candidatesFor({organizationId,executionId}); const candidate=candidates.find(item => item.pathId===pathId && item.findingId===findingId && item.code===code.toUpperCase() && item.resourceId===resourceId);
    if(!candidate) return res.status(409).json({error:'REMEDIATION_CANDIDATE_INVALID'});
    const requestedAction=text(req.body?.action,'ACTION_TEXT_INVALID'); if(requestedAction!==candidate.action) return res.status(409).json({error:'REMEDIATION_ACTION_MISMATCH'});
    const requestedRationale=text(req.body?.rationale || candidate.rationale,'RATIONALE_TEXT_INVALID'); if(requestedRationale!==candidate.rationale) return res.status(409).json({error:'REMEDIATION_RATIONALE_MISMATCH'});
    return res.status(201).json(await proposeRemediation({organizationId,executionId,pathId,findingId,code:candidate.code,resourceId,action:candidate.action,rationale:candidate.rationale,actorId:actor(req),idempotencyKey:req.get('Idempotency-Key')||null}));
  } catch(error){next(error);}
});
router.post('/executions/:executionId/remediations/:remediationId/approve', async (req,res,next)=>{try{const organizationId=org(req);if(!organizationId)return res.status(403).json({error:'ORGANIZATION_CONTEXT_REQUIRED'});authorize(req);const executionId=validId(req.params.executionId,'EXECUTION_ID_INVALID');const remediationId=validId(req.params.remediationId,'REMEDIATION_ID_INVALID');const current=(await listRemediations({organizationId,executionId})).find(item=>item.id===remediationId);if(!current)throw new Error('REMEDIATION_NOT_FOUND');if(!canApproveRemediation({code:current.code,role:role(req)})){const error=new Error('REMEDIATION_APPROVAL_AUTHORITY_REQUIRED');error.status=403;throw error;}return res.json(await approveRemediation({organizationId,executionId,remediationId,actorId:actor(req),idempotencyKey:req.get('Idempotency-Key')||null}));}catch(error){next(error);}});
...
router.post('/executions/:executionId/remediations/:remediationId/verify', async (req,res,next)=>{try{const organizationId=org(req);if(!organizationId)return res.status(403).json({error:'ORGANIZATION_CONTEXT_REQUIRED'});authorize(req);const executionId=validId(req.params.executionId,'EXECUTION_ID_INVALID');const remediationId=validId(req.params.remediationId,'REMEDIATION_ID_INVALID');const current=(await listRemediations({organizationId,executionId})).find(item=>item.id===remediationId);if(!current)throw new Error('REMEDIATION_NOT_FOUND');if(current.state==='REPORTED_APPLIED')await requestRemediationVerification({organizationId,executionId,remediationId,actorId:actor(req),idempotencyKey:req.get('Idempotency-Key')||null});const verification=await verifyRemediation({organizationId,executionId,remediationId,actorId:actor(req),idempotencyKey:req.get('Idempotency-Key')||null});if(verification.state==='VERIFIED'){try{const reanalysis=await runRemediationReanalysis({organizationId,executionId,remediationId,findingId:verification.findingId,actorId:actor(req),req});return res.json({executionId,remediation:verification,verified:true,reanalysis:{status:'COMPLETED',eventId:reanalysis.eventId,freshScanId:reanalysis.freshScanId,afterPathCount:reanalysis.afterPaths.length,riskBefore:reanalysis.riskBefore,riskAfter:reanalysis.riskAfter}});}catch(reanalysisError){return res.status(202).json({executionId,remediation:verification,verified:true,reanalysis:{status:'INCOMPLETE',errorCode:String(reanalysisError.code||reanalysisError.message||'REMEDIATION_REANALYSIS_FAILED')}});}}return res.json({executionId,remediation:verification,verified:verification.state==='VERIFIED',reanalysis:{status:'NOT_RUN'}});}catch(error){next(error);}});
