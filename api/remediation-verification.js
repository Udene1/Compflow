...
import { withRemediationExecutionLock } from '../core/remediation_execution_lock.js';
...
router.post('/executions/:executionId/remediations/:remediationId/execute', async (req,res,next)=>{try{const organizationId=org(req);if(!organizationId)return res.status(403).json({error:'ORGANIZATION_CONTEXT_REQUIRED'});authorize(req);const executionId=validId(req.params.executionId,'EXECUTION_ID_INVALID');const remediationId=validId(req.params.remediationId,'REMEDIATION_ID_INVALID');return await withRemediationExecutionLock({organizationId,executionId,remediationId},async()=>{const result=await executeApprovedRemediation({organizationId,executionId,remediationId,actorId:actor(req),req,idempotencyKey:req.get('Idempotency-Key')||null});let postcheck=null;try{postcheck=await runRemediationPostcheck({organizationId,executionId,remediationId,actorId:actor(req),req});}catch(error){postcheck={status:'FAILED',errorCode:String(error.code||'POSTCHECK_FAILED')};}const pending=await requestRemediationVerification({organizationId,executionId,remediationId,actorId:actor(req),idempotencyKey:req.get('Idempotency-Key')||null});return res.status(202).json({executionId,remediation:pending,execution:result,postcheck,verified:false,verificationRequired:true});});}catch(error){next(error);}});
