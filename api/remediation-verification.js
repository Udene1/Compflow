...
router.post('/executions/:executionId/remediations/:remediationId/execute', async (req,res,next)=>{try{const organizationId=org(req);if(!organizationId)return res.status(403).json({error:'ORGANIZATION_CONTEXT_REQUIRED'});authorize(req);const executionId=validId(req.params.executionId,'EXECUTION_ID_INVALID');const remediationId=validId(req.params.remediationId,'REMEDIATION_ID_INVALID');const result=await executeApprovedRemediation({organizationId,executionId,remediationId,actorId:actor(req),req,idempotencyKey:req.get('Idempotency-Key')||null});return res.status(202).json({executionId,remediation:result,verified:false,verificationRequired:true});}catch(error){next(error);}});
